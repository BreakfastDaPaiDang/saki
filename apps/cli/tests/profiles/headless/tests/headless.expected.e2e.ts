...
async function deepseekDefaultsServer(
  options: { waitForTitleRequest?: boolean; commentIntervalMs?: number; protocol?: 'messages' } = {},
...
describe('headless stream-json snapshots', () => {
  ...
  it.each([60, 500])('sends DeepSeek defaults through the one-shot app with %i ms provider comment gaps', async (commentIntervalMs) => {
    const server = await deepseekDefaultsServer({ protocol: 'messages', commentIntervalMs })
    try {
      const result = await runLoaderSmoke({

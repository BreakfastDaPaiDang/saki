...
describe.skipIf(MODE === 'record')('web e2e: file and session references through the real host', () => {
  ...
  it('waits for the refined folder to become available before drilling', async () => {
    onTestFailed(() => saveFailureShot(page, 'web-e2e-reference-pending-folder'))
    const input = page.locator('[data-composer-input]').first()
    const menu = page.getByRole('listbox', { name: 'Trigger suggestions' })
    const crumbs = page.getByRole('navigation', { name: 'Folder navigation' })
    await writeComposerDraft(page, input, '@folder')
    await menu.getByRole('option', { name: /^folderx\//, disabled: false }).waitFor()
    let release = (): void => {}
    const gate = new Promise<void>((resolve) => { release = resolve })
    let held = false
    const pattern = '**/api/fileReferences/list'
    await page.route(pattern, async (route) => {
      const request = route.request().postDataJSON() as { payload?: { args?: { query?: string } } }
      if (request.payload?.args?.query === 'folderx') {
        held = true
        await gate
      }
    ...
    try {
      ...
      // Fuzzy results already contain both names checked by the drill path;
      // their continued visibility does not establish pick readiness.
      await menu.getByRole('option', { name: /^folderx\// }).waitFor()
      await menu.getByRole('option', { name: /child\.txt/ }).waitFor()
      const readyFolder = menu.getByRole('option', { name: /^folderx\//, disabled: false })
      await expect.poll(() => readyFolder.count(), { timeout: 15_000 }).toBe(0)
      release()
      await readyFolder.waitFor()
      await page.keyboard.press('Tab')
      await expect.poll(() => input.textContent()).toBe('@folderx/')
      await menu.getByRole('option', { name: /child\.txt/, disabled: false }).waitFor()

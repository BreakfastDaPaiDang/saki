...
interface SdkAssertions {
  /** Exact final model answer required before snapshot write-back. */
  expectedFinalResponse?: string
  /** Exact executed tool names required before snapshot write-back. */
  expectedToolCalls?: readonly string[]
  /** Exact skill invocations committed to the session log. */
  expectedSkillInvocations?: readonly string[]
...
const SDK_ASSERTIONS: Readonly<Record<string, SdkAssertions>> = {
  'saki-skill-pack-handoff': {
    expectedFinalResponse: 'HANDOFF_READY',
    expectedToolCalls: ['read', 'write'],
    expectedSkillInvocations: ['ask-matt', 'handoff'],
  },
  'saki-skill-pack-missing-shell': {
    expectedFinalResponse: 'This workflow requires either the DSH bash or pwsh capability to perform the required tracker preflight. Select a Saki development preset that exposes one of those shell capabilities, then retry. No Work Item was created or edited.',
    expectedToolCalls: [],
    expectedSkillInvocations: ['to-tickets'],
  },
...
async function hydrateReplayFixtures(scenario: CorpusScenario, cwd: string): Promise<string[]> {
  ...
  return Promise.all((await fixtureFiles(scenario)).map(async (source) => {
    ...
    await writeFile(destination, (await readFile(source, 'utf8')).replaceAll('{{cwd}}', portableCwd))
    return destination
  }))
}
...
async function runScenario(scenario: CorpusScenario): Promise<{
  ...
  const workspaceSetup = scenario.manifest.workspace?.setup
  if (workspaceSetup !== undefined) {
    if (workspaceSetup !== 'saki-skill-pack') throw new Error(`${scenario.name}: unknown workspace setup ${workspaceSetup}`)
    await mkdir(join(cwd, '.git'), { recursive: true })
    await cp(join(corpusRoot, '..', '.dsh', 'skills'), join(cwd, '.dsh', 'skills'), { recursive: true })
  }
  const ignoredWorkspaceEntries = workspaceSetup === 'saki-skill-pack'
  ...
  for (const scenario of sdkScenarios) {
    ...
    scenarioTest(`${mode}s ${scenario.name} through dsh --profile sdk`, async () => {
      ...
      if (assertions.expectedFinalResponse !== undefined) {
        expect(results.at(-1)?.finalResponse).toBe(assertions.expectedFinalResponse)
      }
      if (assertions.expectedToolCalls !== undefined) {
        const calls = ordered.flatMap(log => records(log.content))
          .filter(record => record.type === 'tool/call')
          .map(record => (record.data as JsonObject).name)
        expect(calls).toEqual(assertions.expectedToolCalls)
      }

      if (assertions.expectedSkillInvocations !== undefined) {
        const invocations = ordered.flatMap(log => records(log.content))
          .filter(record => record.type === 'user/message')
          .map(record => (record.data as JsonObject).source as JsonObject | undefined)
          .filter(source => source?.kind === 'skill-invocation')
          .map(source => source?.name)
        expect(invocations).toEqual(assertions.expectedSkillInvocations)
      }

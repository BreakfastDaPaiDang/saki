...
- **每个生产提供方 HTTP 请求都携带应用归属头。** 适配器发送 `attributionHeaders()`（见下文）作为 `User-Agent` 基线，并通过协议级测试加以证明。会覆盖该头部的库 carrier 在上游正式版本保留它之前不得由产品采用；当前 pi-ai Codex carrier 正是这种受阻 carrier。

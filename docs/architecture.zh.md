...
`agent/pre-step` 决定模型看到什么。监听器可以改写已领取的消息，也可以直接拒绝它们；首次领取被拒绝或被改写为空时，仍会关闭一个不含步骤的持久轮次，因此日志会记录这次尝试。每个步骤读取插件注册的提示词片段和工具 schema。每个由循环构造的模型请求都携带模型不可见的 Session id 与当前数字轮次；同一轮次中的所有步骤共享该值，下一个持久轮次再将其递增。辅助模型请求可以省略轮次。

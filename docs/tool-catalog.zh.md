...
| --- | --- | --- | --- | --- | --- |
| `@breakfastdapaidang/saki-tool-intervention` | `request_intervention` | `ctx.tools`、`ctx.sessions`、`ctx.sakiControlPlane`、`ctx.timer` | `tool/call`、持久 Saki Intervention Request、`tool/result` | - | 仅供系统自有的 Saki Development Agent Preset 使用。持久写入成功会结束当前轮次；操作员回答稍后会进入同一个 Agent Run。 |
...
当工作因需要决策或信息缺失而无法继续时，向 Saki 操作员请求持久输入。请求持久写入成功后会结束当前轮次；回答稍后会进入同一个 Agent Run。
...
```json
{
  ...
  "properties": {
    "question": {
      "type": "string",
      "description": "One concise question the operator must answer before this Agent Run can continue (1-4096 characters)."
    }
  },
  "required": [
    "question"
...
来源：[`packages/saki/tool-intervention/src/index.ts`](../packages/saki/tool-intervention/src/index.ts)

仅供系统自有的 Saki Development Agent Preset 使用。持久写入成功会结束当前轮次；操作员回答稍后会进入同一个 Agent Run。

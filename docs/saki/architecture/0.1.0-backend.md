...
Before artifact effects, an upgrade journal fixes the source state version, storage-generation identity, and build provenance together with the backup and candidate identities. Upgrade creates and verifies the exact Recovery Backup, migrates a fresh v4 candidate database, opens it with current declarations, validates its storage-generation seal and product relationships, proves the source artifacts unchanged, and atomically publishes a `ready` manifest. A crash before publication retains the selected v2 or v3 authority; a crash after publication reopens v4. Recovery compares the backup with the journal-bound source identity before cleaning either a committed or uncommitted operation. Fresh state instead publishes a v4 `provisioning` manifest before control-plane creation and promotes that exact generation to `ready` after validation.

`saki-installation-maintenance` exposes offline `backup`, explicit `verify`, and exact v2-or-v3-to-v4 `upgrade` through the CLI and PowerShell 7 wrapper. Recovery Backup verification resolves the recorded state version through the reader's capability; source build identity is provenance only. Recovery Backups are exact local rollback artifacts, not portable archives, and the package exposes no restore command.

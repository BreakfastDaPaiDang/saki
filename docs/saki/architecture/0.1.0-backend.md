...
The bundle configures JSON as the default `storageDomain` backend and routes only `saki_control_plane` to its dedicated SQLite database; Session event logs use the separate JSONL persistence provider. Unrelated DSH domains do not share the Saki Installation State Generation lifecycle. Saki-owned persisted schemas register contiguous, deterministic `N -> N+1` migrations with historical input and output validation. The generic migration runner belongs to `storage-domain`, never performs external calls or emits active-domain change events, and preserves the existing fail-loud version mismatch for a domain without a complete chain. Stored versions newer than the current build and downgrade requests reject.

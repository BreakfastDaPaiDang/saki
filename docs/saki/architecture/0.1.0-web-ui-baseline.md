...
| --- | --- | --- | --- |
| Work | Simple; a beginner, occasional contributor, or an operator who wants a quiet view | See relevant work across authorized Projects, submit a request, hand ready work to an Agent, answer a question and accept a result | Plain-language groups for ready to start, active, waiting for you, and recently finished; each item has one next action |
...
[K7 (#51)](https://github.com/BreakfastDaPaiDang/saki/issues/51) is the sole production owner of editable Project Settings controls: default Agent Profile, Automation Policy and typed budgets, automatic-Done evidence, and synchronization configuration. Resource Binding and GitHub mapping appear there only as read-only summaries linked to the K1 registration or rebind flow and K3 mapping or repair flow. K6 consumes the completed section for integration and recovery flows; it does not recreate these controls elsewhere.
...
The default page shows only work relevant to the current Principal, grouped as ready to start, active, waiting for you, and recently finished. These are presentation groups rather than Work Item Status values. Each card names the Project, plain-language task, current actor, last confirmed time, and the recommended Action Offer supplied by the My Work Projection. Card Action Offers include Hand to Agent, Answer, Review, and Open. Submit request is a page action and never consumes an item's single card Action Offer. Automatic mode replaces Hand to Agent with an attributed “Agent is working” state. Work awaiting acceptance stays in waiting for you; a Done or Canceled item never shows Review or acceptance.
...
- `main` is a root-scoped keyed slot rendered in the AppFrame main column. The reserved `conversation` entry stays mounted while a selected Saki address shows `saki:work` or `saki:project`; DSH packages do not import Saki types.
- `sidebar.primary.action` is a root-scoped list slot below New Session and above the Workspace browser. This location belongs to daily product destinations, so Saki contributes Work and Project there and the sidebar continues to own their geometry and collapsed presentation. It is distinct from the existing `sidebar.footer.action`, which remains at the bottom beside Settings for utility extensions.

...
组合配置把 JSON 设为默认 `storageDomain` backend，并把 Saki 产品状态 domain 路由到一份专用 SQLite generation；Session 事件日志使用独立的 JSONL 持久化提供方。无关 DSH domain 不共享 Saki Installation State Generation 生命周期。状态格式 capability 与 build 来源相互独立：精确 B03 v2 包含不带 seal 的 `saki_control_plane@2`，保留的 v3 包含 `saki_control_plane@3` 与 `saki_storage_generation@1`，保留的 v4 包含 `saki_control_plane@4` 与 `saki_storage_generation@2`，当前 v5 包含 `saki_control_plane@5`、`saki_host_execution@1` 与 `saki_storage_generation@3`。通用 `storage-domain` runner 会校验连续 `N -> N+1` 声明、精确源与目标记录和已提交读回，而且不发送活动 domain change event。确定性与不产生外部 effect 仍是登记调用方的义务：同步 callback 只获得 detached、冻结 snapshot，但运行时校验无法检测 ambient dependency。没有完整 chain 的 domain 保留版本不匹配快速失败；向下迁移与运行中 capability 不支持的状态版本都会拒绝。

v4 source reader 拥有一份封闭且冻结的精确历史 id、limit、grammar、schema、fingerprint 与 helper 集合。可变的当前 schema 只校验迁移后的 v5 output，因此后续产品演进无法悄然收窄保留的 v4 input language。
...
在产物副作用前，升级 journal 会固定源状态版本、storage-generation identity 与 build 来源，并固定 backup 和 candidate identity。升级会创建并验证精确 Recovery Backup、迁移全新 v5 candidate 数据库、通过当前声明打开该数据库、校验三个当前 domain、storage-generation seal 与产品关系、证明源产物未改变，再原子发布 `ready` manifest。发布前崩溃时，选中的 v2、v3 或 v4 保持权威；发布后崩溃时重新打开 v5。无论 operation 是否已提交，恢复都会先将 backup 与 journal 绑定的源 identity 比对，再执行清理。全新状态则会在创建当前 domain 前发布 v5 `provisioning` manifest，并在校验后把同一精确 generation 提升为 `ready`。

`saki-installation-maintenance` 通过 CLI 与 PowerShell 7 包装脚本暴露离线 `backup`、显式 `verify`，以及精确 v2、v3 或 v4 到 v5 的 `upgrade`。Recovery Backup 验证会通过 reader capability 解析记录的状态版本；源 build identity 只表示来源。Recovery Backup 是精确本地回滚 artifact，不是可移植 archive，而且该包不暴露 restore 命令。
...
- Migration contract test 覆盖精确保留 v2、v3 与 v4 准入、历史关系拒绝、通往 v5 的每项相邻 migration、chain gap、无效 output、不受支持的磁盘状态版本和向下迁移拒绝。Current-leaf drift test 会替换可变 schema、limit 与 helper，而封闭且冻结的 v4 source pack 仍会接受完整的保留 table 集合。Maintenance test 覆盖所有保留升级入口、lease 排他、Recovery Backup 完整性、journal 绑定的源 identity、源不变、candidate 校验、CLI 行为，以及 journal、backup、candidate、invariant 与 manifest 发布周围的 crash injection；系统只会重新打开恰好一个由 manifest 选中的 generation。Export 与替换 Host restore 加入后，需要自己的认证加密、声明排除项、Session 纳入与恢复状态 evidence。

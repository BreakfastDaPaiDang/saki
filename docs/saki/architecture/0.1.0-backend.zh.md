...
直接结构化 Git 路径已经使用由 Control Intent 拥有的 source-general Host Operation。本节的 Execution Dispatch、Agent Run、Dispatch Claim、fencing-token 与 Execution Lease 协议属于下一项 execution 增量，而不是已实现 B07 行为；它会以 `execution-dispatch` 与 `agent-run` source 扩展 Host operation-source union，并复用已有 Host Operation 生命周期。
...
HostExecution
  ...
  readDiff(request, signal) -> ReadProjectDiffResult
  prepareOperation(request, signal) -> HostOperationReceipt
  startOperation(acceptance, admission, signal) -> HostOperationStartResult
  inspectOperation(reference, signal) -> HostOperationSnapshot | undefined
  cancelOperation(reference, reason, signal) -> HostOperationSnapshot | undefined
  onChanged(listener) -> disposer
```
...
Stage 与 unstage 会生成并验证 target alternate index；Commit 从精确已观察 index tree 创建确定性、无 hook 且无签名的 object。每项 mutation 都会在任何真实 `index.lock` 出现前，通过持久 `not-started` plan 记录已绑定 index 同目录的完整随机 operation-owned pin：index operation 固定 target index 字节，Commit 固定其精确 lock marker。Attempt 会从该 pin 创建一条不替换现有文件的 hard link 作为 `index.lock`。已经规划的 mutation 会在持久化语义终态或 reconciliation state 前，移除其 blocking lock 或证明该 lock 不存在并同步 index 目录；终态 replay 会重试对非阻塞 pin 与 scratch 目录的精确 owner 清理。Attached-HEAD planning 会固定目标 branch；Host 在副作用前立即重新验证 HEAD，并只对该固定 ref 执行 compare-and-set，因此之后并发发生的 checkout 无法重定向 publication。Detached HEAD 仍可读取、查看 Diff、stage 与 unstage；但 CreateCommit 不可用，因为 Git 2.45 无法在 compare-and-set object id 的同时原子证明 `HEAD` 始终是 direct ref。持久 `not-started`、`attempting` 与 `applied-recorded` publication evidence 会区分已证明 target 与副作用前 failure；attempted effect 后出现 expected value 属于 ABA-ambiguous，并会停止等待 reconciliation。Local Host 要求 Git 2.45 或更高版本。
...
- 后续 dispatch 增量的崩溃注入会在 dispatch 持久化、claim、Host preparation、控制面 acceptance、operation start、Intervention Request 与 Lease 转换后停止，重新打开持久状态，并验证恢复过程通过同一 Host Operation 确定继续，不会重复外部副作用或 Agent Run。
- 后续 dispatch 协议测试会拒绝陈旧 claim、改变 replay payload、acceptance 前 start、acceptance 前 cancellation 竞态与迟到 receipt；结果不明保持 reconciliation required。
...
- 已实现结构化 Git 测试覆盖展示安全的完整 status、有界 Diff paging、不含路径的 mutation selection、precondition staleness、Binding Write Admission exclusivity、精确 replay、alternate-index publication、确定性 Commit creation、跨越并发 checkout 的固定 target ref compare-and-set、Git 2.45 admission、transient start、cancellation、crash recovery、精确 scratch ownership 与 unknown-effect reconciliation。

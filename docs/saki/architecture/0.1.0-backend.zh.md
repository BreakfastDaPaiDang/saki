...
组合配置把 JSON 设为默认 `storageDomain` backend，并把 Saki 产品状态 domain 路由到一份专用 SQLite generation；Session 事件日志使用独立的 JSONL 持久化提供方。无关 DSH domain 不共享 Saki Installation State Generation 生命周期。状态格式 capability 与 build 来源相互独立：精确 B03 v2 包含不带 seal 的 `saki_control_plane@2`；保留的 v3、v4 与 v5 包含各自匹配的控制面版本以及 `saki_storage_generation@1`、`@2` 与 `@3`；当前 v6 包含 `saki_control_plane@6`、`saki_host_execution@1` 与 `saki_storage_generation@4`。通用 `storage-domain` runner 会校验连续 `N -> N+1` 声明、精确源与目标记录和已提交读回，而且不发送活动 domain change event。确定性与不产生外部 effect 仍是登记调用方的义务：同步 callback 只获得 detached、冻结 snapshot，但运行时校验无法检测 ambient dependency。没有完整 chain 的 domain 保留版本不匹配快速失败；向下迁移与运行中 capability 不支持的状态版本都会拒绝。

保留的 v4 source pack 冻结其精确历史 id、limit、grammar、schema、fingerprint 与 helper。相邻 v5 domain 也会在 v5 到 v6 的步骤加入 Work Item 记录与恢复 metadata 前，冻结自身 GitHub 同步输入。可变的当前 schema 只校验迁移后的 v6 output，因此后续产品演进无法悄然收窄任一保留输入语言。
...
在产物副作用前，升级 journal 会固定源状态版本、storage-generation identity 与 build 来源，并固定 backup 和 candidate identity。升级会创建并验证精确 Recovery Backup、迁移全新 v6 candidate 数据库、通过当前声明打开该数据库、校验三个当前 domain、storage-generation seal 与产品关系、证明源产物未改变，再原子发布 `ready` manifest。发布前崩溃时，选中的 v2、v3、v4 或 v5 保持权威；发布后崩溃时重新打开 v6。无论 operation 是否已提交，恢复都会先将 backup 与 journal 绑定的源 identity 比对，再执行清理。全新状态则会在创建当前 domain 前发布 v6 `provisioning` manifest，并在校验后把同一精确 generation 提升为 `ready`。

`saki-installation-maintenance` 通过 CLI 与 PowerShell 7 包装脚本暴露离线 `backup`、显式 `verify`，以及精确 v2、v3、v4 或 v5 到 v6 的 `upgrade`。Recovery Backup 验证会通过 reader capability 解析记录的状态版本；源 build identity 只表示来源。Recovery Backup 是精确本地回滚 artifact，不是可移植 archive，而且该包不暴露 restore 命令。
...
直接结构化 Git 使用由 Control Intent 拥有的 source-general Host Operation。本节提议的 Execution Dispatch、Agent Run、Dispatch Claim、fencing-token 与 Execution Lease 协议定义额外的 `execution-dispatch` 与 `agent-run` source，并复用已有 Host Operation 生命周期；这些记录与 admission rule 不属于直接 Git 实现。
...
Project-selection inspection 仍然只读。其请求包含一个选定的 Saki Host id 与一个不可信目录 locator。提供方会在每次调用时独立解析 locator，并返回同一次检查产生的浏览器安全 `ProjectSelectionProjection` 与可信 Host 观察。检查绝不创建 Workspace 或 Resource Binding。Bound-project status、有界 Diff 与 source-general Host Operation 生命周期已经实现；worktree 创建、rebind、repair、retirement、automated dispatch 与 Agent Run 生命周期仍属于独立 Consumer。
...
直接 `StageFiles`、`UnstageFiles` 与 `CreateCommit` Intent 会固定已认证 Actor、authority、Registry 与 Project revision、完整 Git precondition，以及已选 id 或 message。每个 Resource Binding 的一条 Binding Write Admission 要么为 `available`，要么在 `reserved` 或 `accepted` phase 预留一个 `manual-host-operation`。控制面持久预留它、prepare 一条 `{ kind: 'control-intent' }` Host Operation、接受该精确 preparation，并在 storage callback 外启动它。精确 replay 返回同一 operation；不同不可变 input 会 conflict；缺失或矛盾 effect evidence 会进入 reconciliation required。[已实现的结构化 Git 决策](../../../.agents/notes/implemented/architecture/2026-08-28-saki-recoverable-structured-git-operations.zh.md)拥有详细生命周期。
...
- 提议的 dispatch 增量会在 dispatch 持久化、claim、Host preparation、控制面 acceptance、operation start、Intervention Request 与 Lease 转换后执行崩溃注入，重新打开持久状态，并验证恢复过程通过同一 Host Operation 确定继续，不会重复外部副作用或 Agent Run。
- 提议的 dispatch 协议测试会拒绝陈旧 claim、改变 replay payload、acceptance 前 start、acceptance 前 cancellation 竞态与迟到 receipt；结果不明保持 reconciliation required。

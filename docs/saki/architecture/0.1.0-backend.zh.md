...
| `packages/saki/installation-maintenance` | Installation lease、manifest 选择、本地 Recovery Backup、精确 B03 升级与崩溃恢复编排 |
...
升级会创建并验证精确 Recovery Backup、迁移全新 candidate 数据库、通过当前声明打开该数据库、校验其 storage-generation seal 与产品关系、证明源产物未改变，再原子发布 `ready` manifest。发布前崩溃时 v2 保持权威；发布后崩溃时重新打开 v3。副作用前 journal 固定 backup 与 candidate identity，以支持确定性清理，但绝不选择状态。全新状态则会在创建控制面前发布 `provisioning` manifest，并在校验后把同一精确 generation 提升为 `ready`。

`saki-installation-maintenance` 通过 CLI 与 PowerShell 7 包装脚本暴露离线 `backup`、显式 `verify` 与精确 B03 v2→v3 `upgrade`。Recovery Backup 验证会通过 reader capability 解析记录的状态版本；源 build identity 只表示来源。Recovery Backup 是精确本地回滚 artifact，不是可移植 archive，而且该包不暴露 restore 命令。
...
- Migration contract test 覆盖精确 B03 v2 准入、历史关系拒绝、chain gap、无效 output、不受支持的磁盘状态版本和向下迁移拒绝。Maintenance test 覆盖 lease 排他、Recovery Backup 完整性、源不变、candidate 校验、CLI 行为，以及 journal、backup、candidate、invariant 与 manifest 发布周围的 crash injection；系统只会重新打开恰好一个由 manifest 选中的 generation。Export 与替换 Host restore 加入后，需要自己的认证加密、声明排除项、Session 纳入与恢复状态 evidence。

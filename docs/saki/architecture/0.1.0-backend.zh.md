...
| Generation Job | route、prompt 与 input 引用、queue 状态、provider attempt、output、请求归属信息 与 cancellation |
...
组合配置把 JSON 设为默认 `storageDomain` backend，并把 Saki 产品状态 domain 路由到一份专用 SQLite generation；Session 事件日志使用独立的 JSONL 持久化提供方。无关 DSH domain 不共享 Saki Installation State Generation 生命周期。状态格式 capability 与 build 来源相互独立：精确 B03 v2 包含不带 seal 的 `saki_control_plane@2`；保留的 v3 与 v4 分别把各自控制面版本和 `saki_storage_generation@1` 与 `@2` 配对；保留的 v5 与 v6 在各自控制面版本及 `saki_storage_generation@3` 或 `@4` 旁包含 `saki_host_execution@1`；保留的 v7 包含 `saki_control_plane@7`、`saki_host_execution@2` 与 `saki_storage_generation@5`；保留的 v8 包含 `saki_control_plane@8`、`saki_host_execution@3` 与 `saki_storage_generation@6`；当前 v10 包含 `saki_control_plane@10`、`saki_host_execution@5` 与 `saki_storage_generation@8`。通用 `storage-domain` runner 会校验连续 `N -> N+1` 声明、精确源与目标记录和已提交读回，而且不发送活动 domain change event。确定性与不产生外部 effect 仍是登记调用方的义务：同步 callback 只获得 detached、冻结 snapshot，但运行时校验无法检测 ambient dependency。没有完整 chain 的 domain 保留版本不匹配快速失败；向下迁移与运行中 capability 不支持的状态版本都会拒绝。

保留的 v4 source pack 冻结其精确历史 id、limit、grammar、schema、fingerprint 与 helper。相邻 v5 domain 也会在 v5 到 v6 的步骤加入 Work Item 记录与恢复 metadata 前，冻结自身 GitHub 同步输入。v6 到 v7 的步骤会加入手动 Agent 记录、默认 Agent Profile reference 与 Give-to-Agent Grant action；Host v1 到 v2 会加入 `StartAgentRun`，同时保留既有 Git operation request 与 evidence。v7 到 v8 的步骤会加入持久 Intervention Request 及其 answer Dispatch graph；Host v2 到 v3 会加入带归因的 intervention-answer input source，同时保留精确 v2 operation。v8 到 v9 的步骤会加入 Branch Delivery 与 Milestone Delivery 记录；Host v3 到 v4 会加入结构化 Push 与精确 Commit 检查，同时保留精确 v3 operation。v9-to-v10 校验历史 Agent 预留，把 Host 准入修订号记录到各自的 Dispatch 并清除 Agent 的 Binding 预留，同时保留手动 Git 预留与精确 Host、Session 和 Run evidence。可变的当前 schema 只校验迁移后的 v10 output，因此后续产品演进无法悄然收窄任何保留输入语言。
...
在产物副作用前，升级 journal 会固定源状态版本、storage-generation identity 与 build 来源，并固定 backup 和 candidate identity。升级会创建并验证精确 Recovery Backup、迁移全新 v10 candidate 数据库、通过当前声明打开该数据库、校验三个当前 domain、storage-generation seal 与产品关系、证明源产物未改变，再原子发布 `ready` manifest。发布前崩溃时，选中的 v2 至 v9 保持权威；发布后崩溃时重新打开 v10。无论 operation 是否已提交，恢复都会先将 backup 与 journal 绑定的源 identity 比对，再执行清理。v10 恢复路径接受当前 v10 journal 目标，或通过相应 frozen domain、seal 与 Agent/Host 链接校验的 v8 或 v9 目标；更早的可读版本仍只能作为 upgrade source。全新状态则会在创建当前 domain 前发布 v10 `provisioning` manifest，并在校验后把同一精确 generation 提升为 `ready`。

`saki-installation-maintenance` 通过 CLI 与 PowerShell 7 包装脚本暴露离线 `backup`、显式 `verify`，以及精确 v2 至 v9 到 v10 的 `upgrade`。Recovery Backup 验证会通过 reader capability 解析记录的状态版本；源 build identity 只表示来源。Recovery Backup 是精确本地回滚 artifact，不是可移植 archive，而且该包不暴露 restore 命令。
...
直接结构化 Git 使用由 Control Intent 拥有的 source-general Host Operation。手动 Give-to-Agent 会在该生命周期中加入 `execution-dispatch` source 与 `StartAgentRun` request。其短期 Dispatch Claim 对交付进行 fencing，每条已接受 Dispatch 保留自己的 Host 准入修订号。独立 Run 可以与其他 Run 及手动 Git 操作共用 Binding。手动路径会在持久接受前要求当前 LLM adapter 解析 Agent Profile 的精确 provider/model route；解析失败会返回 `model-route-unavailable`，不会保留 Agent operation 记录或启动生成。解析只证明已登记的 provider adapter 与精确 model metadata，不证明生产 provider authorization、credential availability、quota 或 account health。后续 Intervention 回答会在同一个 Run、Work Session 与物理 Session 上使用一条新有序 Dispatch 和稳定 MessageId，并独立完成 Dispatch 准入。已实现恢复顺序由[手动 dispatch](../../../.agents/notes/implemented/feature/2026-08-18-saki-manual-give-to-agent-dispatch.zh.md)与[持久 Intervention](../../../.agents/notes/implemented/feature/2026-08-18-saki-durable-intervention-answer.zh.md)决策说明。
...
直接 `StageFiles`、`UnstageFiles` 与 `CreateCommit` Intent 会固定已认证 Actor、权限、Registry 与 Project 修订号、完整 Git 前置条件，以及已选 id 或消息。每个 Resource Binding 的一条 Binding Write Admission 要么为 `available`，要么在 `reserved` 或 `accepted` 阶段预留一次直接 Git 或 Branch Push 操作。直接 Git 会准备一条 `{ kind: 'control-intent' }` Host Operation、接受该精确准备结果，并在存储回调外启动它。Agent 输入使用独立的 Dispatch 准入。精确重放返回同一操作；不同的不可变输入会冲突；缺失或矛盾的副作用证据会进入 reconciliation required。[已实现的结构化 Git 决策](../../../.agents/notes/implemented/architecture/2026-08-28-saki-recoverable-structured-git-operations.zh.md)拥有直接 Git 生命周期。
...
Release Intent 在 finalization 前写入准确官方 upstream Commit，作为 Upstream Baseline。带 expected revision 的 finalization Intent 通过有界且不含凭据的 read 验证配置的 public upstream Repository 中的该 Commit：`GET /repos/{owner}/{repo}` 必须匹配已持久化的规范 `nameWithOwner`、Repository node id、database id 与 public visibility，然后轻量 `GET /repos/{owner}/{repo}/git/commits/{commit_sha}` read 必须返回完整且准确的 SHA。该匿名路径使用独立串行队列，并受 GitHub 共享的低额未认证配额约束；0.1.0 不支持 private upstream Repository。Intent 会验证 baseline 是递归 peel 后 Release Commit 的祖先，并在同一 Milestone Delivery record 中原子嵌入不可变 Release Evidence，其中包含 tag、Release、Commit、baseline、此前 Milestone Delivery revision 与支持 evidence。Branch name 与 `target_commitish` 都不能替代任一 Commit。Finalization 期间观察到的外部 GitHub 变化会使 Intent 进入 conflict 或 reconciliation，且不会发布局部 phase 或 evidence mapping。
...
Request 指定 Generation Job 与 Intent、已解析 Model Route、prompt、input Attachment reference、输出约束和 请求归属信息。Provider adapter 读取验证过的输入，并在返回 output reference 前通过 `ctx.attachments` 保存完成的输出字节。它不写入 Project worktree；用户可见的写入由独立 Host `MaterializeArtifact` operation 完成。
...
到达或离开模型的图片 prompt、reference-image input、选中 output 与 请求归属信息，必须在 UI 或另一个 Agent 消费前表示为可重建 Work Session event 或持久 reference。模型可调用的生图 skill 是此 seam 的 Consumer，不是后端本身。

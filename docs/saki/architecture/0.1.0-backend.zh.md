...
首次登记接收预期 Registry revision，以及浏览器确认的检查指纹与继承变更 baseline。控制面在创建 Workspace 前持久化登记 Intent，把保留的规范 worktree 路径作为不可信 locator 重新检查，再观察或接纳准确 Workspace 身份，并通过比较并设置把 Project、Resource Binding、路径索引与 Intent 映射提交到一个 Registry revision。重启与准确 Intent 重放会从已记录阶段继续，收敛到同一个回执以及相同的 Project、Binding 与 Workspace 身份；payload 变化、Registry revision 陈旧和路径身份重复会产生冲突。[已实现登记 Agent Note](../../../.agents/notes/implemented/architecture/2026-08-20-saki-existing-directory-project-registration.md)拥有这一有界生命周期。

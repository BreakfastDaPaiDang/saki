...
在产物副作用前，升级 journal 会固定源状态版本、storage-generation identity 与 build 来源，并固定 backup 和 candidate identity。升级会创建并验证精确 Recovery Backup、迁移全新 v7 candidate 数据库、通过当前声明打开该数据库、校验三个当前 domain、storage-generation seal 与产品关系、证明源产物未改变，再原子发布 `ready` manifest。发布前崩溃时，选中的 v2 至 v6 保持权威；发布后崩溃时重新打开 v7。无论 operation 是否已提交，恢复都会先将 backup 与 journal 绑定的源 identity 比对，再执行清理。全新状态则会在创建当前 domain 前发布 v7 `provisioning` manifest，并在校验后把同一精确 generation 提升为 `ready`。

`saki-installation-maintenance` 通过 CLI 与 PowerShell 7 包装脚本暴露离线 `backup`、显式 `verify`，以及精确 v2 至 v6 到 v7 的 `upgrade`。Recovery Backup 验证会通过 reader capability 解析记录的状态版本；源 build identity 只表示来源。Recovery Backup 是精确本地回滚 artifact，不是可移植 archive，而且该包不暴露 restore 命令。

...
`credentials-windows-dpapi` Provider 以通用 DSH package 实现现有凭据 Service Definition。它只把受管理值保存为可版本化、不透明 Host-local 文档中的 CNG DPAPI `LOCAL=user` 密文，并在 `CredentialInfo` 与 `ResolvedCredential` 中报告由 Provider 定义的 `protectionLevel`。每个类型化提供方都构造明确等级，解析器则在构造提供方结果前校验无类型输入；任何默认值都不得把未知来源标记为受保护。
...
原始解析保留在有特权的 Host Provider consumer 中。控制面与 Host 读取响应接收安全目录 View，其中包含 reference、source、保护等级、配置状态、可写性、健康和观测时间，但没有 resolve method。Agent tool、动态 execution context、Session event、Projection、log、diagnostic 与读取响应都不接收原始值或密文。具名的只写 `credentials.set` 请求是唯一携带明文的 Host 凭据传输，而且绝不会回显该值。Saki 组合层另行把凭据材料排除在崩溃收集与可携带导出之外。这些规则防止意外访问，却不会把 DPAPI 变成同用户进程隔离；`local-user-trust` 正是对这项限制的命名。

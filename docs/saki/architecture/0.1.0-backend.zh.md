...
在产物副作用前，升级 journal 会固定源状态版本、storage-generation identity 与 build 来源，并固定 backup 和 candidate identity。升级会创建并验证精确 Recovery Backup、迁移全新 v4 candidate 数据库、通过当前声明打开该数据库、校验其 storage-generation seal 与产品关系、证明源产物未改变，再原子发布 `ready` manifest。发布前崩溃时，选中的 v2 或 v3 保持权威；发布后崩溃时重新打开 v4。无论 operation 是否已提交，恢复都会先将 backup 与 journal 绑定的源 identity 比对，再执行清理。全新状态则会在创建控制面前发布 v4 `provisioning` manifest，并在校验后把同一精确 generation 提升为 `ready`。

`saki-installation-maintenance` 通过 CLI 与 PowerShell 7 包装脚本暴露离线 `backup`、显式 `verify` 与精确 v2 或 v3 到 v4 的 `upgrade`。Recovery Backup 验证会通过 reader capability 解析记录的状态版本；源 build identity 只表示来源。Recovery Backup 是精确本地回滚 artifact，不是可移植 archive，而且该包不暴露 restore 命令。
...
该 Service Definition 被有意限制为只读。其请求包含一个选定的 Saki Host id 与一个不可信目录 locator。提供方会在每次调用时独立解析 locator，并返回同一次检查产生的浏览器安全 `ProjectSelectionProjection` 与可信 Host 观察。检查绝不创建 Workspace 或 Resource Binding。Binding 范围修改、worktree 管理、repair、退役、dispatch 准备与 operation 生命周期方法需要单独设计的 Consumer 操作，不会作为占位接口出现在本服务中。

...
`credentials-windows-dpapi` Provider 以通用 DSH package 实现现有凭据 Service Definition。它只把受管理值保存为可版本化、不透明 Host-local 文档中的 CNG DPAPI `LOCAL=user` 密文，并在 `CredentialInfo` 与 `ResolvedCredential` 中报告由 Provider 定义的 `protectionLevel`。每个类型化 Provider 都构造明确等级，parser 则在构造 Provider 结果前校验无类型输入；任何默认值都不得把未知来源标记为受保护。
...
原始解析保留在有特权的 Host Provider consumer 中。控制面与 Host 读取响应接收安全目录 View，其中包含 reference、source、保护等级、配置状态、可写性、健康和观测时间，但没有 resolve method。Agent tool、动态 execution context、Session event、Projection、log、diagnostic 与读取响应都不接收原始值或密文。具名的只写 `credentials.set` 请求是唯一携带明文的 Host 凭据 transport，而且绝不会回显该值。Saki composition 另行把凭据材料排除在崩溃收集与可携带 export 之外。这些规则防止意外访问，却不会把 DPAPI 变成同用户进程隔离；`local-user-trust` 正是对这项限制的命名。
...
- Windows 凭据测试验证静态数据采用 CNG DPAPI `LOCAL=user` 密文且描述符真实，拒绝真正的经典与机器作用域 blob，固定明确保护元数据和失败关闭的来源 policy，并证明 Host 安全读取响应不含明文或密文。测试另行验证只写 `credentials.set` 请求不会回显；产品崩溃收集与 export 测试拥有各自的 composition 排除项。

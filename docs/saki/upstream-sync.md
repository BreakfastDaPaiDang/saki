...
When upstream changes system prompts, model requests, Session formats, or preset composition, include the [assembled Saki Agent Run expected-output test](../../scripts/saki-agent-run.expected.e2e.ts) and both `saki-skill-pack` cases in the [SDK snapshot owner](../../snapshots/sdk/sdk.snapshot.ts) in local validation. Preserve committed Session generations and require these checks to retain Saki behavior when adapting test consumers to upstream APIs.

Publish repairs to `automation/upstream-sync` with an exact lease against the fetched pull request head. A moved remote ref stops publication and requires reconciliation. Keep the pull request draft during iteration; after local checks, mark it ready and wait for the current head's `all checks passed` check to conclude `success` before enabling auto-merge with `--merge --match-head-commit <tested-head>` through `gh pr merge --repo BreakfastDaPaiDang/saki --auto`. Upstream synchronization uses a merge commit to retain official history. Verify the merged result and the Issue's completion before taking the next target.

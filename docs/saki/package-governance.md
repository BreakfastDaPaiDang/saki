...
The current package family is listed in [`packages/saki`](../../packages/saki/README.md). `@breakfastdapaidang/saki-bundle` keeps an empty root config and owns the declared patch that composes local access, existing-directory inspection, first Project registration, structured Git reads and direct mutations, GitHub Board synchronization and Work Item mutations, and one readiness row. The storage domain uses JSON by default and routes current `saki_control_plane@6`, `saki_host_execution@1`, and `saki_storage_generation@4` into the same manifest-selected SQLite generation; Session logs retain their separate JSONL persistence. `pnpm run saki` is a repository development entry; it does not read or replace a user's `start-dsh-with-clash.ps1` or other host-local startup wrapper.

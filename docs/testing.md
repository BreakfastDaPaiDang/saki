...
- Product smokes launch through `dsh` using built artifacts under plain Node ([launch rule](architecture.md#application-launch)), exposing failures tsx masks: settle races, module resolution, and swallowed load failures. Non-index runtime entries and shared singleton modules also need built-artifact smokes (`packages/ptc-runtime/ptc-runtime-node/tests/built-lib.e2e.ts`, `packages/sdk/server/tests/built-scope-carrier.e2e.ts`). Assert missing configuration exits non-zero.

...
| --- | --- | --- | --- | --- | --- |
| `@breakfastdapaidang/saki-tool-intervention` | `request_intervention` | `ctx.tools`, `ctx.sessions`, `ctx.sakiControlPlane`, `ctx.timer` | `tool/call`, `durable Saki Intervention Request`, `tool/result` | - | Private to the system-owned Saki Development Agent Preset. Durable success concludes the current turn; the operator answer arrives later in the same Agent Run. |
...
Request durable input from the Saki operator when work cannot continue without a decision or missing information. A successful request ends the current turn; the answer arrives later in the same Agent Run.
...
Source: [`packages/saki/tool-intervention/src/index.ts`](../packages/saki/tool-intervention/src/index.ts)

...
Saki 在分支采用本历史机制之前已发行三种消息来源变体，[Saki 来源导入记录](2026-09-19-saki-message-sources.zh.md) 对其作出确认。只有该记录的三个精确前后 schema 摘要对允许这一历史联合类型差异保留版本；改变后的 schema 与所有后续转换仍遵循通常规则。

...
The Actor must be constructed inside the trusted control plane. Accepting a caller-supplied Actor or inferring authority from a browser payload, Session lineage, Agent object, GitHub organization membership, or possession of a credential would turn source attribution into permission and make non-escalation unenforceable.
...
Signals carry source attribution but no authority. A Project Automation Principal needs both an explicit Grant and a satisfied Automation Policy before it can submit an Intent. A one-off Agent Run receives only a delegated subset of its initiating Principal's authority and cannot grant itself or a child more access. A durable Agent Identity may receive its own Grants, but its model output never changes them directly.

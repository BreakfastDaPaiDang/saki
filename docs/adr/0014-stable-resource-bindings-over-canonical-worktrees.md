...
A Development Project and every Execution Lease address one stable Resource Binding id. A local binding stores a revisioned observation of its DSH Workspace and Git worktree location, but no path, branch, remote, or Git object id becomes the binding's identity. Registration canonicalizes the available directory and Git administrative paths to prevent aliases from creating a second writable binding. Relocation and replacement-Host recovery update the observation only through an attributed rebind operation.
...
Git distinguishes the main worktree from linked worktrees through per-worktree administrative data. `git rev-parse --git-dir` identifies that private administrative location, while `--git-common-dir` identifies the shared repository family. Those paths are strong duplicate-detection evidence while the worktree is available, but they can change when a main repository is moved, copied to another Host, or reconstructed from a clone. Git does not provide a portable repository UUID that Saki can treat as permanent authority.
...
Version 0.1.0 registers an existing directory. It does not create, move, remove, prune, or repair a Git worktree. The Host resolves the directory with `fs.realpath`, verifies that it is a directory, and asks Git for the absolute top level, per-worktree Git directory, common Git directory, HEAD, branch or detached state, object format, and remotes. It parses `git worktree list --porcelain -z` rather than localized human output and confirms that the selected top level belongs to one reported worktree.

The Host canonicalizes every existing filesystem location returned by Git with `fs.realpath`. It does not lowercase paths because a Windows directory may enable case-sensitive semantics. A candidate collides with an available binding on the same Host when either its canonical worktree root or canonical per-worktree Git directory matches. The common Git directory groups related worktrees but does not make them the same binding.
...
Host tests cover drive-case aliases, separators, `..`, symlinks, junctions, linked worktrees sharing one common directory, detached HEAD, missing paths, moved main and linked worktrees, Git repair, replacement clones, dirty registration, restart, stale binding revisions, rebind quiescence, and preserved historical Sessions. The Git mechanisms follow the official [git-worktree](https://git-scm.com/docs/git-worktree) and [git-rev-parse](https://git-scm.com/docs/git-rev-parse) interfaces.

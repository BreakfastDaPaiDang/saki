...
4. 控制面针对当前未过期 claim 以 compare-and-set 接受 Host Operation reference。系统重新检查 cancellation、委派权限、适用的 Automation Policy，然后把 dispatch 转为 `accepted`。
...
6. Claim 过期、释放或替换会使先前 claimant 失效，但不会取消已接受 Host Operation、停止其 Execution 。系统通过另行授权的 inspect 与 cancel Control Intent 控制 operation。
...
手动启动与自动启动使用同一协议；Automation Policy 只改变谁能提交源 Intent，不改变交付安全属性。
...
**在完整 Execution 期间持有 Dispatch Claim。** Agent Run 的持续时间可能远长于 dispatch admission，而且一个 Run 可以执行多个 Host operation。长时间 claim 会推迟恢复，并把独立命令交付耦合到一个 Session 生命周期。

/**
 * Generate `THIRD_PARTY_NOTICES.md` from every external dependency named by
 * workspace `package.json` files, the pinned Saki Development Skill Pack source
 * in `.dsh/skill-pack/manifest.json`, the vendored-package manifest in
 * `vendor/README.md`, the Python `pyproject.toml` files, the pnpm workspace and
 * patch declarations, and explicit build-time tool pin sources. License and
...
This file lists **direct** dependencies declared by the workspace, the pinned source behind the Saki Development Skill Pack, and the explicitly disclosed official Claude platform payload closure. It is generated by \`scripts/gen-third-party-notices.ts\` from the workspace manifests, \`.dsh/skill-pack/manifest.json\`, \`vendor/README.md\`, the Python project manifests, the pnpm workspace and installed dependency metadata, and the explicit build-time tool pin source. A pre-commit hook regenerates it whenever a staged repository input or the generator changes, and \`scripts/gen-third-party-notices.spec.ts\` asserts in the test lane that the committed bytes match. Deleting a manifest runs no hook, so that case is caught by the assertion instead. Run \`pnpm run verify-third-party-notices\` for the standalone check.

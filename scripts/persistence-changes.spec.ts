...
describe('persistence change classification', () => {
  it.each(['agent/inbox/spliced', 'session/title-llm-request', 'user/message'])(
    'imports only the sealed historical Saki sources for %s', (event) => {
      const read = (id: string): PersistenceRoot => {
        const snapshot = parsePersistenceSnapshot(JSON.parse(readFileSync(
          new URL(`../docs/persistence-changes/${id}.schema.json`, import.meta.url), 'utf8',
        )))
        const root = snapshot.roots.find(root => root.key === `event:${event}`)
        if (root === undefined) throw new Error(`missing historical root ${event}`)
        return root
      }
      const before = read('2026-09-14-image-offload')
      const after = read('2026-09-19-saki-message-sources')
      const messagePath = event === 'agent/inbox/spliced' ? 'inserted[].' : event === 'session/title-llm-request' ? 'messages[].' : ''
      expect(classifyPersistenceChange(before, after)).toEqual([{
        path: `event:${event}.data.${messagePath}source`,
        kind: 'union-variants-changed', description: 'union variants changed', requiresVersionBump: false,
      }])
      expect(classifyPersistenceChange(read('2026-09-11-initial'), after).some(change => change.requiresVersionBump)).toBe(true)
      const nodes = [...after.schema.nodes]
      const envelope = nodes[0]
      if (envelope?.kind !== 'object') throw new Error('historical event envelope must be an object')
      const dataIndex = envelope.properties.find(property => property.name === 'data')?.type
      const data = dataIndex === undefined ? undefined : nodes[dataIndex]
      if (dataIndex === undefined || data?.kind !== 'object') throw new Error('historical event body must be an object')
      nodes[dataIndex] = { ...data, properties: [...data.properties, { name: 'requiredProbe', type: nodes.length, optional: false }] }
      nodes.push({ kind: 'primitive', type: 'string' })
      const schema = canonicalizeSchema(nodes, 0)
      const changed = { ...after, schema, digest: schemaDigest(schema) }
      expect(classifyPersistenceChange(before, changed).some(change => change.requiresVersionBump)).toBe(true)
      expect(classifyPersistenceChange(after, before).some(change => change.requiresVersionBump)).toBe(true)
    },
  )

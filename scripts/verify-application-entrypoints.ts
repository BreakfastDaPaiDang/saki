/**
 * Enforce dsh profiles and the explicit private Saki launcher inventory.
...
/** Product launchers, offline Saki maintenance, and the private WebWorker packer. */
const MANIFEST_BIN_ALLOWLIST = new Map<string, ManifestBin>([
  ['apps/cli/package.json', { dsh: 'lib/bin.js' }],
  ['packages/saki/bundle/package.json', { saki: 'lib/bin.js' }],
  ['packages/saki/installation-maintenance/package.json', { 'saki-maintenance': 'lib/bin.js' }],
...
if (process.argv[1] !== undefined && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
  ...
  if (failures.length > 0) {
    ...
    console.log('verify-application-entrypoints: all application, build, and test launchers are classified.')
  }
}

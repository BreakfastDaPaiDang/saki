...
describe('application entrypoints', () => {
  ...
  it.each([
    ['packages/saki/bundle/package.json', 'saki'],
    ['packages/saki/installation-maintenance/package.json', 'saki-maintenance'],
  ])('accepts only the classified Saki bin at %s', (path, command) => {
    const root = fixture()
    write(root, path, JSON.stringify({ bin: { [command]: 'lib/bin.js' } }))
    expect(applicationEntrypointViolations(root)).toEqual([])

    write(root, path, JSON.stringify({ bin: { [command]: 'lib/other.js' } }))
    expect(applicationEntrypointViolations(root)).toEqual([
      expect.stringContaining(`${path}: classified bin must remain`),
    ])
  })

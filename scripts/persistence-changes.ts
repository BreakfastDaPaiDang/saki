...
// These exact transitions import Saki message sources that predate this history mechanism.
// The sealed schemas preserve the shipped format; later changes use the ordinary version rules.
const RETAINED_SAKI_SOURCE_TRANSITIONS = new Map([
  ['event:agent/inbox/spliced', {
    before: '646c2f1d243d3a78c5bc9305786fb340f5209c48a97e65f2a186cc5195491f3f',
    after: '3bf1f018545e4a456dc441c96fbe30cd396d1068dd3c9aed12f60ef7fa92f66f',
  }],
  ['event:session/title-llm-request', {
    before: 'e0b5bf44c27bbfc6ab144239e3e4169c4d21146645e0e3121bbb359c3bc8591d',
    after: 'b7db8b7a915c06cec46b3cbdb97c048a65bdd9aa8e88648d6d9d8ec8ce8710fa',
  }],
  ['event:user/message', {
    before: '314765bdff29c7862fb6ce820f1773563ba3094a680d163ea21180a2591b8578',
...
/** Classify structural differences, including the sealed import of pre-existing Saki message sources.
...
export function classifyPersistenceChange(before: PersistenceRoot | null, after: PersistenceRoot | null): PersistenceTypeChange[] {
  ...
  const retained = RETAINED_SAKI_SOURCE_TRANSITIONS.get(key)
  const importsRetainedSources = before.key === key && before.kind === 'event' && after.kind === 'event'
    && before.surface === after.surface
    && retained?.before === before.digest && retained.after === after.digest
  const classified = changes.map(change => importsRetainedSources && change.kind === 'union-variants-changed'
    ? { ...change, requiresVersionBump: false }
    : change)
  return [...new Map(classified.map(change => [JSON.stringify([change.path, change.kind, change.requiresVersionBump]), change])).values()]
}

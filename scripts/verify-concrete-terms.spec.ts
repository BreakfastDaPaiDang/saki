...
describe('concrete terminology policy', () => {
  ...
  it('retains exact Saki replay evidence and rejects changed or unlisted recordings', () => {
    const file = 'snapshots/sdk/saki-skill-pack-handoff/session.v2.jsonl'
    const source = readFileSync(new URL(`../${file}`, import.meta.url), 'utf8')
    expect(source.toLowerCase()).toContain(blockedTerm)
    expect(findConcreteTermViolations(file, source)).toEqual([])
    expect(findConcreteTermViolations(file, `${source}\n`)).not.toEqual([])
    expect(findConcreteTermViolations(file.replace('v2', 'v4'), source)).not.toEqual([])
    expect(findConcreteTermViolations(`copy/${file}`, source)).not.toEqual([])
  })

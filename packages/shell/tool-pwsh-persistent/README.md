...
PowerShell starts with `-NoLogo -NoProfile -NonInteractive`. This keeps the persistent runspace and native child terminal input while disabling automatic PSReadLine loading and rejecting PowerShell-host prompts such as `Read-Host`. Explicit `shellArgs` must match that supported list; redirected-stdin modes are rejected. ConsoleHost can echo submitted input; complete tool results remove wrapper echoes, while bounded partial output may retain them.

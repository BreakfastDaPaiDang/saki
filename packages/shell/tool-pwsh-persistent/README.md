...
- **The supported PTY path echoes submitted input.** The marker-anchored extraction excludes the echo in complete results; the wrapper-source strip covers fallback paths, but a wrapper that wraps across the terminal width may leave a partial echo in partial-output results, bounded by `maxOutputChars`.
...
- The default backend starts pwsh with `-NonInteractive`: PowerShell-host prompts such as `Read-Host` and confirmation UI fail immediately. This tool supplies no interactive stdin to a native foreground command, so one that reads input blocks until the readiness timeout resets the shell.

...
PowerShell 使用 `-NoLogo -NoProfile -NonInteractive` 启动。它保留持久 runspace 和原生子进程终端输入，同时禁止自动加载 PSReadLine，并拒绝 `Read-Host` 等 PowerShell-host 提示。显式 `shellArgs` 必须匹配该支持列表；重定向 stdin 模式会被拒绝。ConsoleHost 可能回显提交的输入；完整工具结果会移除 wrapper 回显，有界部分输出可能保留它们。

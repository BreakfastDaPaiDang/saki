...
- **受支持的 PTY 路径会回显提交的输入。** 完整结果中 marker 锚定提取排除回显；包装器原文剥离覆盖回退路径，但跨越终端宽度的包装器折行可能在部分输出结果中残留片段回显，受 `maxOutputChars` 约束。
- 模型重定义 `prompt` 函数会移除就绪标记；shell 随后退化为静默档而非 marker 快路径。
- 默认后端以 `-NonInteractive` 启动 pwsh：`Read-Host` 和确认 UI 等 PowerShell 宿主提示会立即失败。本工具不向原生前台命令提供交互 stdin，因此读取输入的命令会阻塞到就绪超时，随后重置 shell。

...
describe('LocalBashExecutor.start (background process handles)', () => {
  ...
  it('kill() requests managed-range termination: true once, false after settlement', async () => {
    const { ctx, bash } = await setup()
    const proc = await bash.start(bash.resolve({ command: 'echo ready; sleep 60' }))
    try {
  ...
  it('a spec.signal abort settles the handle as killed, not completed', async () => {
    const { ctx, bash } = await setup()
    const controller = new AbortController()
    const proc = await bash.start(bash.resolve({ command: 'echo ready; sleep 60', signal: controller.signal }))
    try {
      await readUntil(proc, 'ready', 10_000)
      controller.abort()
      await proc.done

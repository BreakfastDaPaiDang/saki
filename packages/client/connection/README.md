...
A dedicated RPC channel is reserved when its caller registers it; its HTTP route mounts when WebServer becomes available and is withdrawn on dependency loss or caller disposal. Removing a reservation before WebServer appears prevents later mounting. RPC registration options also declare required headers and opaque error responses. The HTTP bridge applies the selected route’s trust policy and header requirements before consuming its body, and carries response headers and status back without exposing thrown values. Saki’s [Host API](../../saki/host-api/README.md) owns bootstrap and authorization semantics.

...
export async function apply(ctx: Context, config?: ConnectionConfig): Promise<void> {
  ...
  const route: WebRoute = {
    ...
    handler: async (req, res) => {
      const options = connection.sharedRequestOptions(req.method ?? 'GET', (req.url ?? '/').replace(/\?.*$/, ''))
      const rejection = options.authority === 'loopback'
        ? isTrustedApiRequest(req, []) ? undefined : 403
        : connection.requestRejection(req)
      if (rejection !== undefined) {
        res.writeHead(rejection, options.requiredResponseHeaders)
        res.end(options.opaqueError?.message ?? (rejection === 401 ? 'unauthorized' : 'forbidden'))
        return
      }
      await bridge(req, res, fetchHandler, maxRequestBodyBytes, {
        ...options.requiredResponseHeaders === undefined ? {} : { headers: options.requiredResponseHeaders },

...
export interface ConnectionRpcHandlerOptions {
  ...
  readonly opaqueError?: ConnectionRpcFailure
}
...
export interface HostConnectionRpc {
  /**
   * Register one authenticated absolute channel prefix. Its HTTP route mounts
   * while a WebServer is available and belongs to the registering caller.
   ...
   * @param options - channel trust and response policy.
   ...
   * @param handler - decoded endpoint handler returning the existing RPC result shape.
   * @param options - interceptor trust and response policy.
...
export interface ClientConnectionRpc {
   ...
   * @param options - cancellation, browser credential policy, and operation headers.

...
function createSessionApi(rpc: ClientConnectionRpc): FixtureSessionApi {
  const call = async <K extends keyof FixtureSessionRequests>(
    ...
    const result = await rpc.call('/api', `session/${remoteEndpoint}`, { args }, signal === undefined ? undefined : { signal })
    return {
      rpcId: request.rpcId,
...
describe('createFixtureApi', () => {
  ...
  it('serves configured DeepSeek readiness and keeps credential values write-only', async () => {
    ...
    const initial = await api.credentialRemote.describe(['DEEPSEEK_API_KEY', 'TEST_API_KEY'])
    if (!initial.ok) throw new Error('credential describe failed')
    ...
    const configured = await api.credentialRemote.describe(['TEST_API_KEY'])
    if (!configured.ok) throw new Error('credential describe failed')
    expect(configured.value).toHaveProperty('TEST_API_KEY', {
      ref: 'TEST_API_KEY',
      configured: true,
      source: 'file',
      protectionLevel: 'plaintext',
    ...
    const cleared = await api.credentialRemote.describe(['TEST_API_KEY'])
    if (!cleared.ok) throw new Error('credential describe failed')
    expect(cleared.value).toHaveProperty('TEST_API_KEY', {
      ref: 'TEST_API_KEY',
      configured: false,
      protectionLevel: 'plaintext',
      writable: true,

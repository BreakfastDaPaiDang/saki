...
Host RPC 方法和 WebSocket stream 默认使用浏览器会话认证。精确 RPC 注册可以为自行负责认证的 Consumer 显式选择 loopback 策略，例如 Saki bootstrap 和 Host RPC。该策略要求 loopback 对端、Host 和匹配的浏览器 Origin，并检查分派后的重定向目标。每个进程生成一个随机启动令牌。`dsh-web-app` 打印并打开带 `?token=...` 的普通根 URL；`frontend-static` 把根路径和 index 请求交给 `ctx.connection.authorizeIndex`，后者只在 `GET /` 接受该令牌，写入绑定 authority 的签名 cookie，再重定向到干净的 `/`。缺失、过期、畸形或 authority 不匹配的 cookie 会在 RPC 分发前得到 401。静态资源保持公开。HTTP 载体不在根路径交换之外接受 query token，也不接受 Authorization header token。
...
独立 RPC channel 在调用方注册时预留；其 HTTP 路由在 WebServer 可用时挂载，并在依赖丢失或调用方释放时撤销。在 WebServer 出现前移除预留会阻止后续挂载。RPC 注册选项还声明必需请求头和不透明错误响应。HTTP bridge 在读取请求体之前应用所选路由的信任策略及请求头要求，并返回响应头和状态而不暴露抛出的值。Saki 的 [Host API](../../saki/host-api/README.zh.md) 负责 bootstrap 和授权语义。

...
Host 插件可以注册专用逻辑 RPC 通道，也可以在 `/api` 上拦截自己拥有的端点。Connection 校验 JSON 请求信封，负责请求关联与 JSON Content-Type，并将取消信号、只读绝对 URL 和只读请求头访问接口与通道载荷分开交给处理方。处理方返回 RPC 结果，还可以在 JSON 信封之外添加 HTTP 响应头。浏览器调用方默认携带同源凭据，并可为单次调用指定取消信号、凭据策略和操作请求头；Connection 会用自己的 `application/json` 覆盖调用方给出的 Content-Type。Connection 只传输这些字段，不解释认证或授权策略。

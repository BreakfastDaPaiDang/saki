...
describe('UiWorkspaceService', () => {
  ...
  it('does not report or select a fork that settles after navigation disposal', async () => {
    const b = bench()
    const gestures = vi.fn()
    b.uiWorkspace.onSessionNavigation(gestures)
    const child = Promise.withResolvers<SessionId>()
    b.sessions.fork.mockImplementation(() => child.promise)
    const pending = b.uiWorkspace.forkSession(sid('row'))
    await b.ctx.fiber.dispose()
    child.resolve(sid('child'))
    await pending
    expect(b.sessions.open).not.toHaveBeenCalled()

...
全局面板占据 root 作用域的 `main` keyed slot；`conversation` 是为会话界面保留的 key。`ctx.layout.selectPanel(id)` 选中已注册面板，`null` 则选中会话界面，但不改变当前会话。其他面板隐藏 Conversation 条目时，该条目仍保持挂载，使用户返回时保留组件状态。默认组合不注册任何全局面板。

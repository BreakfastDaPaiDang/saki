...
function mountFrame(windowWidth = frameWidth) {
  ...
  const renderSlot: AppFrameProps['renderSlot'] = (key, owner, options) => {
    ...
    return <div data-testid={`${key}-content`} data-entry-key={options?.entryKey}>
      {key === 'main' && <input aria-label={`draft-${options?.entryKey}`} defaultValue="draft" />}
    </div>
  }
...
describe('AppFrame', () => {
  ...
  it('preserves the mounted Conversation and draft while another main panel is selected', () => {
    ...
    const draft = conversation.querySelector('input')!
    ...
    for (const panelId of ['panel-a' as MainPanelId, 'panel-b' as MainPanelId, null]) {
      ...
      expect(conversation.parentElement!.hidden).toBe(panelId !== null)

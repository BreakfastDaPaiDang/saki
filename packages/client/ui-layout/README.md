...
Global panels occupy the root-scoped `main` keyed slot; `conversation` is the reserved key for the Conversation. `ctx.layout.selectPanel(id)` selects a registered panel, and `null` selects the Conversation without changing the current Session. The Conversation entry stays mounted while another panel hides it, preserving component state when the user returns. No global panel is registered by the shipped composition.

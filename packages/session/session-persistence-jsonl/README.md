...
- **Platform lock dependency** — `fs-ext` is optional at installation and loads only for POSIX locks; a missing POSIX addon fails lock acquisition. Windows uses its kernel semaphore without loading the addon.

...
describe('RemoteMock.rpc', () => {
  it('calls and opens by endpoint, taking the proxies\' positional args or the Gateway\'s single object', async () => {
    ...
    await expect(mock.rpc.call('/api', '$events/result', { args: { clientId: 'c' } }, { signal: idle() })).resolves.toEqual({ ok: true, value: [{ clientId: 'c' }] })
  ...
  it('rejects an aborted call: immediately when already aborted, with the reason while the rule is pending', async () => {
    ...
    const request = mock.rpc.call('/api', 'slow/call', { args: [] }, { signal: pending.signal })
    pending.abort(new Error('caller left'))
    await expect(request).rejects.toThrow('caller left')
    const already = new AbortController()
    already.abort('not an error')
    await expect(mock.rpc.call('/api', 'slow/call', { args: [] }, { signal: already.signal })).rejects.toThrow('remote-mock: call aborted')
    await expect(mock.rpc.call('/api', 'session/list', { args: [] }, { signal: idle() })).rejects.toThrow('no rule for session/list')

...
import type { CredentialRef, CredentialInfo } from '@deepseek-ai/dsh-credentials/types'
...
class LeakyCredentials extends MemoryCredentials {
  override async describe(ref: CredentialRef): Promise<CredentialInfo> {
    return Object.assign(await super.describe(ref), { value: 'sk-leaked' })
  }
}
...
describe('the credentials Remote namespace a configuration surface calls', () => {
  ...
  it('describes a batch of references as one map, values excluded', async () => {
    ...
    expect(described).toEqual({
      DEEPSEEK_API_KEY: { ref: 'DEEPSEEK_API_KEY', configured: true, source: 'memory', protectionLevel: 'ephemeral', writable: true, health: 'available', observedAt: expect.any(Number) as unknown },
      OPENAI_API_KEY: { ref: 'OPENAI_API_KEY', configured: false, protectionLevel: 'ephemeral', writable: true, health: 'missing', observedAt: expect.any(Number) as unknown },
    })
  ...
  it('answers only the fields the view declares, whatever a provider returns', async () => {
    ...
    expect(described.DEEPSEEK_API_KEY).toEqual({ ref: 'DEEPSEEK_API_KEY', configured: false, protectionLevel: 'ephemeral', writable: true, health: 'missing', observedAt: expect.any(Number) as unknown })
  ...
  it('stores and removes through the same references the batch describes', async () => {
    ...
    expect(await controller.describe(['DEEPSEEK_API_KEY']))
      .toEqual({ DEEPSEEK_API_KEY: { ref: 'DEEPSEEK_API_KEY', configured: true, source: 'memory', protectionLevel: 'ephemeral', writable: true, health: 'available', observedAt: expect.any(Number) as unknown } })
    await controller.unset('DEEPSEEK_API_KEY')
    expect(await controller.describe(['DEEPSEEK_API_KEY']))
      .toEqual({ DEEPSEEK_API_KEY: { ref: 'DEEPSEEK_API_KEY', configured: false, protectionLevel: 'ephemeral', writable: true, health: 'missing', observedAt: expect.any(Number) as unknown } })
  })

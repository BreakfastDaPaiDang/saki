...
function navigationHook() {
  const instance = createSnapshotStore<PanelInfo>({ activePanelId: null })
  const subscribe = (listener: () => void) => instance.subscribe(listener)
  const getSnapshot = () => instance.getSnapshot()
  const usePanelInfo = <S,>(select: (state: ReturnType<typeof getSnapshot>) => S): S =>
    select(useSyncExternalStore(subscribe, getSnapshot))
  return { instance, usePanelInfo }
}

describe('SakiNavEntry', () => {
  it('renders the wide 「工作」 row, marks it current when elected, and opens on click', () => {
    const { instance, usePanelInfo } = navigationHook()
    const open = vi.fn()
    render(<SakiNavEntry wide={true} open={open} sakiSurface="work" usePanelInfo={usePanelInfo} useResource={neverHook} useSessions={neverHook} useSessionPendingInteraction={neverHook} useWorkspaces={neverHook} t={t} />)
    ...
    act(() => { instance.set({ activePanelId: 'saki:work' as MainPanelId }) })
    expect(screen.getByRole('button', { name: '工作' }).getAttribute('aria-current')).toBe('page')
    act(() => { instance.set({ activePanelId: 'saki:project' as MainPanelId }) })
  ...
  it('renders the collapsed 「项目」 rail without the label and tracks the project surface', () => {
    const { instance, usePanelInfo } = navigationHook()
    const open = vi.fn()
    render(<SakiNavEntry wide={false} open={open} sakiSurface="project" usePanelInfo={usePanelInfo} useResource={neverHook} useSessions={neverHook} useSessionPendingInteraction={neverHook} useWorkspaces={neverHook} t={t} />)

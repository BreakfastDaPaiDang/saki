...
- **Browser surface without a model adapter** — the composed shell renders the Saki pages and the Conversation fallback, but production installs no model adapter and the browser's `/api` event sockets are not served (the console shows reconnect noise): Conversation turns stay idle until a later slice wires both.

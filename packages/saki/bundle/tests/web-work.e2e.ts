...
it('retains Run evidence and Conversation navigation across intervention, failure, reload and Host restart', async () => {
  ...
  try {
    ...
    await page.getByRole('button', { name: '查看 Run', exact: true }).first().click()
    await page.getByText('等待用户', { exact: true }).first().waitFor()
    await page.getByRole('button', { name: '执行记录', exact: true }).click()
    await page.getByText('Should this Agent Run preserve the current repository state before continuing?', { exact: false }).waitFor()
    await page.screenshot({ path: join(frames, '02-waiting-run.png') })
    await page.getByRole('button', { name: '前往我的工作处理', exact: true }).click()
    ...
    await page.getByRole('button', { name: '查看 Run', exact: true }).first().click()
    await page.getByText('最近执行成功', { exact: true }).waitFor({ timeout: stepMs })
    await page.getByRole('button', { name: '打开 Session', exact: true }).click()
    ...
    await composer.fill('Keep this Conversation draft while inspecting the Run.')
    await page.getByRole('button', { name: '返回 Run', exact: true }).click()
    await page.getByRole('button', { name: '执行记录', exact: true }).click()
    await page.getByText('Host operation · 输入已送达', { exact: true }).first().waitFor()
    await page.getByRole('button', { name: '打开 Session', exact: true }).click()
    await expect.poll(() => composer.innerText()).toBe('Keep this Conversation draft while inspecting the Run.')
    await composer.fill('Exercise the Run failure observation.')
    await composer.press('Enter')
    await page.getByText('Controlled Run provider failure', { exact: false }).first().waitFor({ timeout: stepMs })
    await page.getByRole('button', { name: '返回 Run', exact: true }).click()
    await page.getByText('最近执行失败', { exact: true }).waitFor({ timeout: stepMs })
    await page.screenshot({ path: join(frames, '06-failed-run.png') })
    await page.reload()
    await page.getByText('最近执行失败', { exact: true }).waitFor({ timeout: stepMs })
    await page.getByRole('button', { name: '终端', exact: true }).click()
    await page.getByText('当前 Agent 未启用 Terminal 服务。', { exact: true }).waitFor()
    await server.stop()
    env.SAKI_PORT = String(port)
    server = startWebServerProcess(process.execPath, ['--import', 'tsx/esm', driver], env, stepMs)
    server.child.stderr.on('data', (chunk: string) => { diagnostics += chunk })
    await server.ready
    await page.reload()
    ...
    await page.getByText('DSH Session 历史可读取，可在 Conversation 查看。后续执行是否可用由当前配置与权限决定。', { exact: true }).waitFor()
    await page.screenshot({ path: join(frames, '07-restarted-run.png') })
    await page.getByRole('button', { name: '打开 Session', exact: true }).click()
    await page.getByText('The existing repository changes are preserved. The requirement is ready for human inspection.', { exact: true }).waitFor()
    await page.getByRole('button', { name: '工作', exact: true }).click()

...
保留的迁移链消费精确 B03 v2-v5 状态，并且只生成当前 v6 记录。其中相邻 v5-to-v6 步骤会加入空的 Work Item Intent 与 targeted-recovery 表、向已置备的 Host Operator 授予 Work Item action，并根据每个 confirmed Board item 的非终态 Status 初始化 `latestNonTerminalStatus`；终态则初始化为 `null`。可变的当前 schema 只校验迁移后的 v6 output。
...
- **只支持人工结构化 mutation**：尚未实现 Resource Binding 重绑定与退役、agent-run write admission、仓库或 Board 自动变更、mapping repair、reconciliation repair 与人工接管。StageFiles、UnstageFiles、CreateCommit、`CreateWorkItem` 与 `MoveWorkItem` 都是显式操作员 Intent；repair overlay 不会自动执行它们，浏览器输入也不能指定仓库路径或 Provider 拥有的 GitHub target id。预期 revision CAS 的失败方会保留已创建或接纳的可复用 DSH Workspace，而不创建 Saki Project 或 Resource Binding；控制面不会删除本次登记可能并非独占的 Workspace。

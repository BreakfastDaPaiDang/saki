...
保留的迁移链读取精确的 v2 至 v9 状态，并生成当前 v10 记录。历史 schema 保持冻结。v9-to-v10 步骤保留 Run、Session、输入与 Host 证据，把 Agent 准入凭据记录到各自的 Execution Dispatch，并清除 Agent 持有的 Binding 预留。直接 Git 与 Branch Push 预留保留原有恢复语义。
...
`branch_deliveries` 把 remote ref、精确 pull request、完整 pull-request review 与精确 Commit CI 保留为相互独立的 targeted source，并分别保存 last-confirmed fact 与当前 health。Review fact 为展示保留原始 GitHub state，绝不进入 transition、acceptance 或 release authority。浏览器 projection 只从已保留的精确 Commit CI fact 派生 `ci.confirmedSummary`，因此当前 failure、stale 状态或 invalidation 仍是权威状态，同时不会抹去上次 confirmed product state。Product App 分离时会取消并排空其 Branch Delivery recovery 批次；单条记录的意外故障会在其余记录分别获得恢复尝试后再报告。

不可变 Milestone Release Evidence 会把准确 Development Project、upstream Repository 的 node/database/规范名称 identity，以及每个选中 Branch Delivery 的规范 head 与 base ref 绑定进自身带摘要的事实。若 evidence 内嵌的 target identity 与 Milestone record 固定的 release expectation 不同，该 record 会拒绝该 evidence。

Board mapping 只使用已配置的 GitHub node id。已配置 Repository 中匹配的 Issue item 按 GitHub Project API order 排列；archived item 变为 Canceled，尚未进入 Project 的开放 Repository Issue 变为 Inbox，并带有明确的非 membership evidence。其他 Repository、pull request、draft item、redacted item、重复 Issue membership、重复派生 Work Item id 与畸形 Provider 关系绝不会进入 confirmed Board。Status field 或 option 缺失时进入可见 mapping repair，不会按名称猜测。每个 confirmed item 都携带 `latestNonTerminalStatus`：非终态 item 记录其当前 Status，Done 或 Canceled 则保留最近已知的非终态 Status；未知时为 `null`。后续完整扫描会合入更新且身份匹配的 targeted-recovery 记忆。关闭的 Issue 仍处于非终态时，Board 派生 `external-close` repair；开放、未 archived 的 Issue 仍处于终态时，则派生 `external-reopen` repair。这些 overlay 建议执行带 Actor 归因的 `MoveWorkItem`，目标为 Done/Canceled 或已记忆的非终态 Status；没有记忆时回退到 Backlog，但绝不会自动修改 GitHub。显式归类和排序已关闭的终态 Issue 会保留其关闭状态；Status 与位置写入根据源观察和已确认的 Issue-state 阶段校验 Issue 状态。
...
规划读取的所有权保留在 Host。`work-item-view` 将当前完整 Board 与定向确认结果关联到持久 Assignment、Run、Intervention、Branch Delivery、已配置 Milestone 及最近 32 条活动引用。完整 Issue 正文的读取可独立失败，并在等待 GitHub 后重新检查 Provider 挂载、准确 Issue 身份、当前 Board 指纹与权限。`project-milestones` 按 Milestone id 分页列出已配置交付摘要；`project-mapping` 在准确同步修订号下发现全部现有字段选项，并报告当前配置权限。字段发现与正文读取均不会发布 Board checkpoint。
...
该模块可选消费 `ctx.sakiGitHub`。没有此 Provider 的 composition 仍能加载，并暴露未配置或缓存状态。Provider 存在时，内含 Consumer 会按 interactive-first 顺序处理持久 scan attempt、只通过 aggregate owner 发布完整候选结果、通过 targeted inspection 恢复可恢复的 Work Item saga，并在 dispose 时取消活跃工作并等待完全停稳。一条限定在 Provider 生命周期内的 loop 会立即执行一次 targeted pending pass，之后在每次 pass 完成后等待 `targetedPendingPollIntervalMs`：Branch Delivery 选择非终态 Intent，以及没有 active Intent 的非 accepted Delivery；Milestone Delivery 只选择已 prepared 的 finalization 与携带 repair 的 reconciliation。交互式 Milestone View 使用当前持久 Board projection 与 targeted read；只有 evaluation 和 final reread 才请求新的完整 Board scan，因此 View 与 pending refresh 绝不推进 Board checkpoint 或 generation。同一 Project 的每次 scan write 都通过同一条逐 Project operation tail 串行，因此其原子存储更新只有一个 writer。逐 Project 同步配置拥有 active 与 background polling interval 及 background rate reserve；控制面插件拥有 attempt lease lifetime 与独立的 targeted-pending interval。完整发布协议参见 [ADR 0013](../../../docs/adr/0013-polling-first-staged-github-synchronization.zh.md)。
...
| `branchDeliveryObservationFreshForMs` | 300000 ms（5 分钟） | 已确认 Branch Delivery evidence 的新鲜度窗口 |

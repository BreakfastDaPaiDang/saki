...
describe('Saki product state versions', () => {
  it('declares exact readable v2 and v3 formats with only v3 writable', () => {
    expect(sakiStateCapability.readable.map(spec => spec.version)).toEqual([2, 3])
    ...
    expect(sakiStateCapability.writable.version).toBe(3)
  ...
  it('seals one v3 storage generation outside the control-plane domain', () => {

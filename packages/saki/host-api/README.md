...
| `control/query` | `{ type: 'milestone-view', projectId, milestoneId, refresh: 'cached' \| 'interactive' }` | Browser-safe Milestone phase, exact scope, and targeted release-source evidence, or typed rejection |
...
| `control/submit` | One of the six revision-fenced Branch Delivery Intents plus request-token header | Safe durable delivery receipt for selection, Push, Pull Request association or creation, review, and acceptance |
| `control/submit` | `save-milestone-delivery` or `finalize-milestone-delivery` Intent plus request-token header | Safe durable Milestone receipt for revision-fenced metadata or exact release-evidence finalization |

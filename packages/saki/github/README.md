...
Targeted inspections retain the exact Issue, Project membership, Status, archive state, relevant API-order neighbors, or Issue open state needed by the corresponding mutation. Project membership inspection distinguishes absent, unique, and duplicate memberships. Position inspection retains the observed moving membership and requested predecessor observation, including an absent anchor, without echoing the request target. Targeted observations never claim or advance a complete Board scan checkpoint.

Issue-create inspection uses the persisted exact hidden marker and classifies a unique actual Issue, complete absence, a pull-request match, a removed marker on a known Issue, a missing known Issue, identity conflict, multiple occurrences, or bounded/inconsistent traversal. Its optional known-Issue hint is inspection-only and does not alter the persisted `operationId`. A complete absence is evidence, not provider permission to create again; the Consumer owns the `effectPossible` decision.

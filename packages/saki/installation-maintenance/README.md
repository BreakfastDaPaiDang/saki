---
description: "Prepare one Saki Installation for serving, inspect its retained state, and perform recoverable offline upgrades with exact Recovery Backups."
...
Prepare one Saki Installation for serving, inspect its retained state, and perform recoverable offline upgrades with exact Recovery Backups.

## Table of Contents

- [Use this package](#use-this-package)
- [State formats and authority](#state-formats-and-authority)
- [Lease, serving, and publication](#lease-serving-and-publication)
...
The state capability, rather than a build id, decides format compatibility. This build can read state versions 2 through 10 and can create only version 10. Build ids in generation and backup metadata record build identity; equality with the running build is never a readability or migration test.
...
`backup` supports a valid selected v2 through v10 generation. `verify` checks one explicit backup id. `upgrade` accepts exact v2 through v9 state and publishes v10 after creating its backup; it rejects state that is already current.
...
- **Eight retained upgrade sources** — the migration path accepts exact B03 v2, post-B18 v3, or retained v4 through v9 and produces v10; it provides no downgrade, reverse migration, or general old-media repair.
...
No runtime invariant companion is published because offline entry points validate durable state while holding the Installation lease.

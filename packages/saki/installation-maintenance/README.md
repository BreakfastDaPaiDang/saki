...
The state capability, rather than a build id, decides format compatibility. This build can read state versions 2 through 8 and can create only version 8. Build ids in generation and backup metadata record provenance; equality with the running build is never a readability or migration test.
...
| 8 | `saki_control_plane@8` plus `saki_host_execution@3` and the required `saki_storage_generation@6` seal | Fresh provisioning, serving, backup, and verification |
...
`backup` supports a valid selected v2 through v8 generation. `verify` checks one explicit backup id. `upgrade` accepts exact v2 through v7 state and publishes v8 after creating its backup; it rejects state that is already current.
...
- **Six retained upgrade sources** — the migration path accepts exact B03 v2, post-B18 v3, or retained v4 through v7 and produces v8; it provides no downgrade, reverse migration, or general old-media repair.

...
The state capability, rather than a build id, decides format compatibility. This build can read state versions 2, 3, and 4 and can create only version 4. Build ids in generation and backup metadata record provenance; equality with the running build is never a readability or migration test.
...
| 4 | `saki_control_plane@4` plus the required `saki_storage_generation@2` seal | Fresh provisioning, serving, backup, and verification |
...
`backup` supports a valid selected v2, v3, or v4 generation. `verify` checks one explicit backup id. `upgrade` accepts exact v2 or v3 state and publishes v4 after creating its backup; it rejects state that is already current.
...
- **Two retained upgrade sources** — the migration path accepts exact B03 v2 or post-B18 v3 and produces v4; it provides no downgrade, reverse migration, or general old-media repair.

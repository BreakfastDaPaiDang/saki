...
The state capability, rather than a build id, decides format compatibility. This build can read state versions 2 through 7 and can create only version 7. Build ids in generation and backup metadata record provenance; equality with the running build is never a readability or migration test.
...
| 7 | `saki_control_plane@7` plus `saki_host_execution@2` and the required `saki_storage_generation@5` seal | Fresh provisioning, serving, backup, and verification |
...
`backup` supports a valid selected v2 through v7 generation. `verify` checks one explicit backup id. `upgrade` accepts exact v2 through v6 state and publishes v7 after creating its backup; it rejects state that is already current.
...
- **Five retained upgrade sources** — the migration path accepts exact B03 v2, post-B18 v3, retained v4, retained v5, or retained v6 and produces v7; it provides no downgrade, reverse migration, or general old-media repair.

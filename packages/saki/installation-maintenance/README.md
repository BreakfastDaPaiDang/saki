...
The state capability, rather than a build id, decides format compatibility. This build can read state versions 2, 3, 4, and 5 and can create only version 5. Build ids in generation and backup metadata record provenance; equality with the running build is never a readability or migration test.
...
| 5 | `saki_control_plane@5` plus `saki_host_execution@1` and the required `saki_storage_generation@3` seal | Fresh provisioning, serving, backup, and verification |
...
`backup` supports a valid selected v2, v3, v4, or v5 generation. `verify` checks one explicit backup id. `upgrade` accepts exact v2, v3, or v4 state and publishes v5 after creating its backup; it rejects state that is already current.
...
- **Three retained upgrade sources** — the migration path accepts exact B03 v2, post-B18 v3, or retained v4 and produces v5; it provides no downgrade, reverse migration, or general old-media repair.

...
The state capability, rather than a build id, decides format compatibility. This build can read state versions 2 through 6 and can create only version 6. Build ids in generation and backup metadata record provenance; equality with the running build is never a readability or migration test.
...
| 6 | `saki_control_plane@6` plus `saki_host_execution@1` and the required `saki_storage_generation@4` seal | Fresh provisioning, serving, backup, and verification |
...
`backup` supports a valid selected v2 through v6 generation. `verify` checks one explicit backup id. `upgrade` accepts exact v2 through v5 state and publishes v6 after creating its backup; it rejects state that is already current.
...
- **Four retained upgrade sources** — the migration path accepts exact B03 v2, post-B18 v3, retained v4, or retained v5 and produces v6; it provides no downgrade, reverse migration, or general old-media repair.

...
格式兼容性由状态 capability 决定，而不是 build id。本 build 可以读取状态版本 2、3、4 和 5，但只能创建版本 5。Generation 与 backup 元数据中的 build id 只记录来源；系统绝不根据它是否等于运行中 build 来判断可读性或可迁移性。
...
| 5 | `saki_control_plane@5` 加 `saki_host_execution@1` 与必需的 `saki_storage_generation@3` seal | 全新配置、对外服务、backup 与验证 |
...
`installation.json` 一旦存在，就是唯一权威。它通过有界 leaf、byte length 与 SHA-256 选中一份准确 `generation.json`；选中的 generation 元数据必须重复 Installation id、storage-generation id 与状态版本。没有 Installation manifest 时，只能选择准确配置的 B03 数据库。两者都不存在时，对外服务流程会配置全新 v5 状态。三个当前产品 domain 共用该 manifest 选中的同一物理 SQLite generation。Generation 名称、backup 时间戳、operation journal 与目录新旧绝不选择状态；无法解释的残留要求进入维护恢复。
...
`backup` 支持有效的已选 v2、v3、v4 或 v5 generation。`verify` 检查一项明确 backup id。`upgrade` 接受精确 v2、v3 或 v4 状态，创建 backup 后发布 v5；状态已经是当前版本时会拒绝。
...
- **保留三个升级源**——迁移路径接受精确 B03 v2、post-B18 v3 或保留 v4 并产生 v5；它不提供降级、反向迁移或通用旧介质修复。

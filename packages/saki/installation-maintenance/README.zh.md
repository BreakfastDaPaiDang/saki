...
格式兼容性由状态 capability 决定，而不是 build id。本 build 可以读取状态版本 2 至 10，但只能创建版本 10。Generation 与 backup 元数据中的 build id 只记录来源；系统绝不根据它是否等于运行中 build 来判断可读性或可迁移性。
...
`installation.json` 一旦存在，就是唯一权威。它通过有界 leaf、byte length 与 SHA-256 选中一份准确 `generation.json`；选中的 generation 元数据必须重复 Installation id、storage-generation id 与状态版本。没有 Installation manifest 时，只能选择准确配置的 B03 数据库。两者都不存在时，对外服务流程会配置全新 v10 状态。三个当前产品 domain 共用该 manifest 选中的同一物理 SQLite generation。Generation 名称、backup 时间戳、operation journal 与目录新旧绝不选择状态；无法解释的残留要求进入维护恢复。
...
`withPreparedSakiServingState()` 会在恢复、状态预检、应用启动、完整 serving callback 与拆除期间一直持有 lease。全新状态会先发布 provisioning manifest，再配置并校验三个当前产品 domain，最后把同一份准确 manifest 提升为 `ready`。有效 v2 至 v9 源会返回 `upgrade-required`；对外服务流程绝不在线迁移它。

升级会先持久发布固定 pending intent、由 identity 选中的不可变 operation journal，再发布固定 active selector；journal 会在产物副作用前固定源状态版本、storage-generation identity 与 build 来源，以及 backup 和 candidate identity。无 manifest 的 v2 恢复会从固定 B03 identity 得到源 build 来源，而不会接受 journal 的自我声明。随后升级创建并验证精确 Recovery Backup、物化独立 v10 candidate、校验其当前 schema 与 Saki 产品关系、证明源 SQLite 产物未改变，最后原子发布 `ready` Installation manifest。无论 operation 是否已经提交，恢复都会先把 backup 与 journal 中的源 identity 精确比对，再清除 operation。Manifest 发布前崩溃时，选中的 v2 至 v9 保持权威；发布后崩溃时，v10 保持权威。v10 build 也会通过相应 frozen domain、seal 与 Agent/Host 链接校验，恢复以 v8 或 v9 为目标的 active journal；v2 到 v7 仍是 upgrade source，不是 active journal 目标。启动与每条维护命令都会在 lease 下调和确定性文件临时项以及 pending、active、settled operation 元数据；这些元数据只用于校验和清理具名产物，绝不用于选择权威。
...
`backup` 支持有效的已选 v2 至 v10 generation。`verify` 检查一项明确 backup id。`upgrade` 接受精确 v2 至 v9 状态，创建 backup 后发布 v10；状态已经是当前版本时会拒绝。
...
- **保留八个升级源**——迁移路径接受精确 B03 v2、post-B18 v3 或保留 v4 至 v9 并产生 v10；它不提供降级、反向迁移或通用旧介质修复。

...
Each mutation has a read-only inspection path that returns only `{ snapshot, observedAt }`. Issue-create inspection first proves the Repository node, database, and owner identities, then traverses `GET /repos/{owner}/{repo}/issues` with `state=all`, fixed created-order pagination, raw Markdown media, and authoritative `Link rel="next"` termination. It scans every admitted page before classifying exact marker occurrences, distinguishes REST pull-request entries through the `pull_request` field, and returns typed incomplete evidence for page/item bounds, malformed pagination, or duplicate entries. Membership inspection traverses the complete Project item connection and reports absent, unique, or duplicate Issue membership. Status and position inspections retain the exact Issue, membership, field option, archive state, API order, and relevant neighbors while rejecting inconsistent target revisions. Issue-state inspection reads the exact Repository and Issue and admits only a repository-bound open or closed fact. These targeted inspections do not produce a Board scan candidate or advance a GitHub Sync Checkpoint.

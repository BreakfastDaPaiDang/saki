...
Each resolution produces one **immutable** snapshot — the profiles plus a `createModels()` collection holding the `Provider` each route built — and every operation captures a whole snapshot before its first `await`. A configuration change builds a *new* collection rather than mutating the one in use: `Models.streamSimple()` resolves its provider lazily, when the stream is first consumed, which is after credential resolution, so a mutated collection would let a request that started under one configuration finish under another or fail on a provider that no longer exists. This is what makes the seam's per-step call freeze (`llm.prepareCall()`) hold end to end — switching models mid-reply takes effect on the next step, never inside the one in flight. Requests reach their provider through `Models.streamSimple()`. A catalog route that keeps its catalog protocol **reuses** the installed provider with its model list replaced, because that provider owns API implementations this package cannot reconstruct — Bedrock loads its Smithy module through a separate entry point — so rebuilding it from parts would silently narrow which providers work. Every other route is built by `createProvider()` over the protocol table behind `supportedProtocols()`, whose entries are the same factories pi-ai's own provider factories use.

The shipped configurable-provider plugin resolves a route's key through the Harness credential seam before the request reaches pi-ai and passes it as the request's `apiKey` option, which remains the highest-priority auth override. A specialized `PiAiAdapter` owner may instead inject one pi-ai `CredentialStore`; every immutable `Models` collection receives that exact store. The owner, not this package, supplies login, persistence, refresh, and credential protection. A route naming no credential in the generic plugin resolves as configured-but-keyless and leaves the requirement to the protocol, which is where it actually lives.

Each stream call also captures provider, model, optional Session id, and optional turn before its first asynchronous wait. `prepareTransportRequest` receives only that identity and may asynchronously return dynamic headers plus an `onResponse` observer. The adapter awaits preparation once, copies its headers immediately, and forwards the observer result for pi-ai to await before body consumption. Preparation rejection prevents provider dispatch; observer rejection follows pi-ai's provider-error stream. Profile resolution rejects `session-id`, `thread-id`, `x-client-request-id`, `x-codex-turn-state`, and `user-agent` case-insensitively because those values belong to the request or Harness attribution. Dynamic headers merge after resolved authentication and deployment headers, then Harness attribution merges last.
...
The adapter supplies the shared attribution header from dsh-llm's `attributionHeaders()` through pi-ai's final header transformation. Generic provider fixtures verify the resulting `User-Agent` on the wire. The Codex carrier in the current pi-ai dependency overwrites that value, so no production consumer may treat Codex carrier adoption as complete until an upstream release preserves it and passes an exact-wire fixture. Provider-specific app-attribution headers are not synthesized. See [dsh-llm § App attribution](../llm/README.md#app-attribution-attributionts).
...
- **The shipped configurable-provider plugin does not offer a provider that authenticates through OAuth alone** — pi-ai resolves OAuth from a stored OAuth credential, while this plugin injects no store and runs no login flow. The configurable-provider directory therefore withholds such routes; `openai-codex` is the only one in the installed catalog. A specialized owner can construct `PiAiAdapter` with a store and its own authorization lifecycle, but that does not make OAuth configurable through the generic settings surface. A route a settings document already names keeps its entry so a configuration surface can edit or delete it, and `apiKeyEnv` still authenticates it with that key — which for Codex is a token that expires with nothing in the generic plugin to refresh it.
- **Codex carrier adoption waits for an upstream release** — the current pi-ai dependency overwrites a caller `User-Agent` and does not provide the complete request identity required by the product integration. This repository carries no package-manager patch or alternate transport for that carrier; its specialized consumer remains deferred until an official release supplies equivalent behavior and exact-wire verification passes.

...
每次解析产出一份**不可变**快照——profiles 加上一个持有各路由所建 `Provider` 的 `createModels()` 集合——每个操作都在自己第一个 `await` 之前整体捕获一份快照。配置变化会构造**新**集合，而不是改动正在被使用的那个：`Models.streamSimple()` 是惰性的，它在流首次被消费时才解析 provider，而那已在凭据解析之后，因此改动共享集合会让一个在旧配置下开始的请求在新配置下结束，或者撞上一个已不存在的 provider。这正是 seam 的每步调用冻结（`llm.prepareCall()`）能贯通到底的原因——回复途中切换模型会在下一步生效，绝不会影响在途的那一步。请求经 `Models.streamSimple()` 抵达提供方。保持 catalog 协议不变的 catalog 路由会**复用**已安装提供方，只替换其模型列表，因为该提供方持有本包无法重建的 API 实现——Bedrock 经由独立入口加载其 Smithy 模块——从零件重建会静默收窄可用提供方的范围。其余路由都由 `createProvider()` 基于 `supportedProtocols()` 背后的协议表构造，表中条目正是 pi-ai 自己的提供方工厂所用的同一批 factory。

已交付的可配置提供方插件会在请求抵达 pi-ai 之前经 Harness 凭据 seam 解析路由密钥，并作为请求的 `apiKey` 选项传入；这仍是优先级最高的 auth 覆盖。专用 `PiAiAdapter` 所有者也可以注入一份 pi-ai `CredentialStore`；每个不可变 `Models` 集合都会收到该精确存储。登录、持久化、刷新与凭据保护由所有者而非本包负责。通用插件中没有点名凭据的路由会解析为「已配置但无密钥」，把该要求留给协议——那才是它真正所在的位置。

每次流调用还会在首次异步等待前捕获提供方、模型、可选 Session id 与可选轮次。`prepareTransportRequest` 只接收这份身份，可以异步返回动态头部与 `onResponse` 观察器。适配器只等待一次准备结果，随后立即复制头部，并把观察器结果交给 pi-ai 在消费响应 body 前等待。准备被拒绝会阻止提供方派发；观察器被拒绝会遵循 pi-ai 的提供方错误流。profile 解析会不区分大小写地拒绝 `session-id`、`thread-id`、`x-client-request-id`、`x-codex-turn-state` 和 `user-agent`，因为这些值属于请求或 Harness 归属。动态头部在已解析认证与部署头部之后合并，Harness 归属最后合并。
...
适配器会通过 pi-ai 的最终头部转换提供 dsh-llm `attributionHeaders()` 的共享归因标头；通用提供方 fixture 会验证最终线路上的 `User-Agent`。当前 pi-ai 依赖中的 Codex carrier 会覆盖该值，因此在上游正式版本保留它并通过精确线路 fixture 之前，任何生产消费方都不得把 Codex carrier 采用视为完成。不会合成提供方特定应用归因标头。详见 [dsh-llm § 应用归因](../llm/README.md#app-attribution-attributionts)。
...
- **已交付的可配置提供方插件不提供仅以 OAuth 认证的提供方**：pi-ai 从已存储 OAuth 凭据解析这类认证，而该插件不注入存储，也不运行登录流程。可配置提供方目录因此不列出这类路由；已安装 catalog 中只有 `openai-codex` 属于此类。专用所有者可以用存储和自己的授权生命周期构造 `PiAiAdapter`，但这不会让通用 settings 界面获得 OAuth 配置能力。settings 文档已经写过的路由仍保留目录条目，配置界面据此可以编辑或删除；`apiKeyEnv` 也仍能用该密钥完成认证——对 Codex 而言那是一个会过期、且通用插件没有任何环节会去刷新的 token。
- **Codex carrier 采用等待上游正式版本**：当前 pi-ai 依赖会覆盖调用方 `User-Agent`，也不提供产品集成所需的完整请求身份。本仓库不为该 carrier 携带包管理器 patch 或替代传输；其专用消费方继续暂缓，直到官方版本提供等效行为并通过精确线路验证。

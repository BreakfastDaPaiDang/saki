...
Product package namespaces are published `@deepseek-ai/dsh-*` and private `@breakfastdapaidang/saki-*`; Cordis `Service` subclasses and function plugins contribute through `ctx.effect()`, `ctx.on()`, or `ctx.waterfall()`. Rules: [package](AGENTS.md), [root](../AGENTS.md#conventions), and [Saki package governance](../docs/saki/package-governance.md).

## Hierarchy

Groups hold `packages/<group>/<pkg>/`; DSH names stay `@deepseek-ai/dsh-<pkg>`, while packages under `packages/saki/` use `@breakfastdapaidang/saki-<pkg>`. **Group READMEs own package/ctx-key maps.**

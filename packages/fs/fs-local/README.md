...
The package-root API is the default/named `LocalFileSystem` class plus `Config` and the Windows-only `readFileDaclWin32`, `copyFileDaclWin32`, and `replaceFileWin32` helpers for trusted host-side DACL inspection, DACL copy, and file replacement. Raw I/O lives in `src/fsio.ts` (Cordis-free, independently unit-tested); `src/index.ts` is the thin service wiring.

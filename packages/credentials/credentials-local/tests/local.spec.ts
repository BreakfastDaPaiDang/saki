...
describe('document writes', () => {
  ...
  it('refuses writes after disposal', async () => {
    ...
    await expect(service.describe(KEY)).resolves.toMatchObject({ configured: true, writable: false })
    await expect(service.describe(OTHER)).resolves.toMatchObject({ configured: false, writable: false })

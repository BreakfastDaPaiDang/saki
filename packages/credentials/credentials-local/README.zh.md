...
这是审慎，不是边界。[`dsh-credentials-windows-dpapi`](../credentials-windows-dpapi/README.zh.md) 是在 Windows 上提供静态加密的平级包，但当前用户作用域仍然信任以同一 Windows 用户身份刻意运行的进程。要让这些进程也无法取得提供方密钥，需要独立隔离的 Credential Broker 或外部机密管理器。

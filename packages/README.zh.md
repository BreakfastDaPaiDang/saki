...
harness 由 `packages/` 下的 npm 包组装而成，按能力系列分组：会话与 agent 循环、面向模型的工具、shell 与文件系统执行、Web 访问、subagent 等等。把本页当作顶层地图使用：先找到拥有某能力的组，再打开其 README 查看包列表。DSH 包使用 `@deepseek-ai/dsh-*`，私有 Saki 包使用 `@breakfastdapaidang/saki-*`；每个包只属于一个组；每个组的 README 都是该能力系列的权威包映射。

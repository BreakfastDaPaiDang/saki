...
`resolveExecutable(command, env?, signal?)` resolves a bare name or verifies an absolute executable in the provider’s execution world. Consumers pass the resolved path to `spawn`; paths and the working directory must describe that same world.

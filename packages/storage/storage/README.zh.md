...
可选 `kv.closed` facet 为不改变介质的检查、精确描述符读取和仅创建式物化保留 unit 名称。reservation 持续到回调及每个已准入的 lease 方法结束；live unit 会立即冲突。提交 token 保留 durable 或 uncertain 发布证据，供同一 lease 回读。[存储子系统](../../../docs/subsystems/storage.zh.md#the-backend-contract) 规定失败与恢复要求。

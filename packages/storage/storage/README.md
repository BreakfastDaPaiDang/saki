...
The optional `kv.closed` facet reserves a unit name for nonmutating inspection, exact-descriptor reading, and create-only materialization. The reservation remains owned until its callback and every admitted lease method settle; a live unit conflicts immediately. Commit tokens preserve durable or uncertain publication evidence for same-lease readback. The [storage subsystem](../../../docs/subsystems/storage.md#the-backend-contract) owns failure and recovery requirements.

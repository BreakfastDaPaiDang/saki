...
`single` 布局使用独立于 domain 版本的严格物理格式 v1，并在写入和读取时校验无损 JSON。它固定配置根目录的身份，拒绝符号链接、junction、畸形文档和已检测到的替换。其 closed-unit lease 支持检查，并通过同目录硬链接实现仅创建式物化。发布结果或持久性不确定时会 poison live handle；恢复前必须关闭并重新创建 backend。[存储子系统](../../../docs/subsystems/storage.zh.md) 规定完整的失败与回读规则。

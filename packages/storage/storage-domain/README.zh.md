...
Cold operation 必须显式调用：`materialize` 创建缺失目标，`defineDomainMigrations` 为 `migrate` 捕获连续且冻结的 schema 链。它们保留 domain 和 backend 名称，保持源介质不变，并在发布成功前校验完整的目标回读。普通 `open` 不执行 cold migration。`invalidRecords: 'backup-and-skip'` 是可丢弃 per-record 数据的独立策略；权威记录和 global slot 仍显式报告校验失败。详见 [cold migration 约定](../../../docs/subsystems/storage.zh.md)。

...
const expectedPath = fileURLToPath(new URL(
  './snapshots/credentials-windows-dpapi/host-safe-view.expected.json',
  import.meta.url,
))
const portableExpectedPath = fileURLToPath(new URL(
  './snapshots/credentials-windows-dpapi/host-safe-view-portable.expected.json',
  import.meta.url,
))
...
describe.runIf(process.platform === 'win32')('Windows DPAPI Provider public Loader snapshot', () => {
  it('loads through an app process and persists only an authenticated current-user record', async () => {
    ...
    expect(Object.keys(document).sort()).toEqual(['records', 'version'])
    expect(document.version).toBe(1)
    expect(Object.keys(document.records ?? {})).toEqual([ref])
    expect(document.records?.[ref]?.kind).toBe('dpapi-ng-local-user')
    expect(document.records?.[ref]?.ciphertext).toBeTypeOf('string')

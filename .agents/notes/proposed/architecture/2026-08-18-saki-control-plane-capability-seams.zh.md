...
已实现的[已有目录 Project 登记](../../implemented/architecture/2026-08-20-saki-existing-directory-project-registration.zh.md)提供只读 Project-selection operation、Local Host Provider 基础与首个控制面 Consumer 操作。已实现的[结构化 Git 决策](../../implemented/architecture/2026-08-28-saki-recoverable-structured-git-operations.zh.md)以已绑定状态、有界 Diff、直接 mutation 与持久 Host Operation 扩展同一 Host Execution seam。已实现的[polling-first GitHub 同步](../../implemented/architecture/2026-08-18-saki-polling-first-github-synchronization.zh.md)提供 GitHub Service Definition、Product App Provider、可恢复 polling Consumer、同步配置与 confirmed Board Projection。已实现的[可恢复 GitHub Work Item mutation](../../implemented/architecture/2026-08-16-saki-recoverable-github-work-item-mutations.zh.md)增加类型化写入、targeted inspection，以及可恢复 CreateWorkItem 与 MoveWorkItem Consumer。本 Agent Note 保持 proposed，因为 automated dispatch-sourced Host operation、剩余 GitHub 写入与 mapping repair，以及 Model Account 和 Image Generation seam 尚未实现。
...
把 Saki 权威控制状态通过 `storageDomain` 路由到专用 SQLite 数据库。通用可选 schema migration 继续由 `storage-domain` 拥有；`installation-maintenance` 产品 plugin 按[已实现维护决策](../../implemented/architecture/2026-08-26-saki-manifest-selected-installation-maintenance.zh.md)拥有 Saki 专属 generation 切换、Recovery Backup 与精确 B03 升级。加密 Installation Export、restore、retention 与替换 Host 恢复仍位于[有效可迁移性提案](2026-08-18-saki-forward-migrations-and-installation-maintenance.zh.md)。这项所有权分离不增加第五个外部 capability seam。

...
One small Installation manifest records the Installation id, active generation id, Saki state-format version, and an exact integrity reference to its generation metadata. The generation metadata records creator-build provenance. Each generation is a complete dedicated Saki database. Exactly one manifest-selected generation is active and writable; a candidate and every retained or backed-up generation remain closed and immutable.
...
2. Close the active Saki domain and database. Create an owner-readable Recovery Backup with the active generation id, exact state-format version, source-build provenance, length, and cryptographic digest, then verify that artifact before migration continues.

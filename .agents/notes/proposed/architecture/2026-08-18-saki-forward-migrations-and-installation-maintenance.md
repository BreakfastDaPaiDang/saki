...
The generic layer is implemented by the [callback-scoped closed-domain migration decision](../../implemented/architecture/2026-08-25-callback-scoped-closed-domain-migration.md). The Saki-specific lease, manifest selection, Recovery Backup, v2-to-v3 upgrade, and crash recovery are implemented by the [manifest-selected Installation-maintenance decision](../../implemented/architecture/2026-08-26-saki-manifest-selected-installation-maintenance.md). This proposal remains active for encrypted export, explicit Recovery Backup restore, retention, and replacement-Host restore.
...
The domain layer reserves and inspects the closed stored unit through generic storage operations, reads and validates a detached snapshot through its matching historical descriptor without opening or changing the source, applies one migration step, and validates the complete output against the next version before continuing. A step receives JSON-compatible records and returns new JSON-compatible records; its declared input contains no backend, Cordis context, credential resolver, clock, network client, or mutable domain handle. Determinism and absence of external effects are caller obligations because the runtime cannot detect ambient imports. Migration never emits ordinary `domain/changed` events because no active domain observes the candidate.

The final snapshot is written into a fresh target unit and read back through the current schema while both closed-unit reservations remain held. The Saki maintenance layer later opens that candidate through the normal current `DomainSpec` and validates product invariants before publication. Existing runtime write ordering, post-durability events, and record operations remain unchanged. A stored version newer than the running version, a missing step, invalid source data, invalid output, unknown table, or target invariant failure aborts without changing the selected generation. The migration API exposes structured version and validation evidence to maintenance code without exposing backend-specific rows.

### Installation State Generations

One small Installation manifest records the Installation id, active generation id, Saki state-format version, and an exact integrity reference to its generation metadata. The generation metadata records creator-build identity. Each generation is a complete dedicated Saki database. Exactly one manifest-selected generation is active and writable; a candidate and every retained or backed-up generation remain closed and immutable.
...
2. Close the active Saki domain and database. Create an owner-readable Recovery Backup with the active generation id, exact state-format version, source-build identity, length, and cryptographic digest, then verify that artifact before migration continues.
...
4. Validate Installation identity, referential integrity, unique admission owners, lifecycle enums, non-terminal Intent and Dispatch recoverability, and the absence of secret values. Write and fsync a candidate manifest, then atomically replace the active manifest.
...
On startup, the manifest is the only selector. A candidate file, backup timestamp, or numerically newest generation never wins implicitly. A crash before the manifest replacement selects the old generation; a crash after replacement selects the new generation. If the manifest or selected generation fails integrity checks, startup enters maintenance recovery and never guesses another writer. Rollback explicitly installs a build whose declared state capability reads the backup's state version and restores that Recovery Backup; the recorded source build is build identity that may identify a candidate, never the compatibility gate. Reverse migration is unsupported.

### Recovery Backup and Installation Export

A Recovery Backup serves local rollback and preserves one exact Saki Installation State Generation. Its recorded state version determines reader compatibility, while its source build identifies build identity only; the artifact is not accepted as a portable Host-transfer claim. It contains only the dedicated Saki database and maintenance metadata; Host credential stores remain outside the generation. Owner-only filesystem protection and verified hashes are required even though the Saki database contains references rather than raw credentials.
...
- Recovery Backup verification detects truncation, digest mismatch, unsupported state versions, missing metadata, and attempts to use the artifact as a portable restore.

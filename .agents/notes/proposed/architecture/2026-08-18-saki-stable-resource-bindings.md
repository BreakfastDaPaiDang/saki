...
The proposed [domain KV storage and Workspace](2026-07-24-domain-kv-storage-and-workspace.md) note owns DSH Workspace `fs.realpath` uniqueness and immutable records; this proposal owns Saki's higher-level binding identity, Git observation, and rebind semantics. The proposed [Installation maintenance](2026-08-18-saki-forward-migrations-and-installation-maintenance.md) note owns replacement-Host restore and links `needs-rebind` into this lifecycle.

The implemented [existing-directory Project registration](../../implemented/architecture/2026-08-20-saki-existing-directory-project-registration.md) establishes the first stable Project and Resource Binding ids, duplicate worktree identity checks, and startup revalidation. The implemented [structured-Git decision](../../implemented/architecture/2026-08-28-saki-recoverable-structured-git-operations.md) revalidates an exact active Binding and its revision for bound status, Diff, and direct mutation, and serializes direct Git mutations through one durable admission record per Binding.

The [manual Give-to-Agent decision](../../implemented/feature/2026-08-18-saki-manual-give-to-agent-dispatch.md) admits each Dispatch independently and permits Agent Runs to share one directory with distinct Sessions. Rebind, retirement, repair, successor Sessions, and physical worktree lifecycle remain proposed.
...
- Alias spellings of one available worktree cannot create two Resource Bindings.

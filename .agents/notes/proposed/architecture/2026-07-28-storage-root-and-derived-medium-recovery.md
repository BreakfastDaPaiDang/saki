...
> Scope update (2026-08-25): resolving the JSON root once and guarding its filesystem identity have shipped; [Callback-scoped closed-domain migration](../../implemented/architecture/2026-08-25-callback-scoped-closed-domain-migration.md) and the current [storage subsystem documentation](../../../../docs/subsystems/storage.md) are the current authority. This note remains proposed only for the independent derived-medium reset design.
...
**Where the files actually live (shipped).** The shared base defaults the session store to the global harness home (`$DSH_HOME/sessions`, default `~/.dsh/sessions`), while the shipped Web overlay used to give the json backend the relative root `./.storages`: `workspace.json` and `session_projcache.json` landed under `<launch dir>/.storages/` — two launches from different directories shared their sessions yet saw different workspace registries and different projection caches. The overlay now anchors `storage-json.root` to `$DSH_HOME/storages`, and `JsonStorageBackend` resolves its configured root once at construction and pins the observed directory identity. Current behavior is documented by the sources linked above.
...
### One global storage root and construction-time resolution (shipped)
...
- **Shipped**: `JsonStorageBackend` resolves its configured root once at construction (`resolve(config.root)`), so a later `process.cwd()` change cannot split one backend across roots, and it guards the identity of the observed root before and after publication.
...
- A domain cannot combine `'reset'` with the migration registry proposed for [Saki forward-compatible state](2026-08-18-saki-forward-migrations-and-installation-maintenance.md). Derived media reset; authoritative compatible state migrates or rejects, and migration failure never triggers deletion.
...
- `dsh` launched from any directory reads and writes the same `$DSH_HOME/storages/*.json` (default `~/.dsh/storages`) — satisfied by the overlay expression and construction-time root resolution; per-row overrides ride the personal config.yaml patch layer.

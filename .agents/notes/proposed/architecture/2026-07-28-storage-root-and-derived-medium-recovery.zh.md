...
> 范围更新（2026-08-25）：JSON 根目录构造时解析与文件系统身份保护均已落地；[callback-scoped closed-domain migration](../../implemented/architecture/2026-08-25-callback-scoped-closed-domain-migration.zh.md)及当前[存储子系统文档](../../../../docs/subsystems/storage.zh.md)是现行权威。本 Note 仅为独立的派生介质重置设计保持 proposed。
...
**文件到底存在哪（已落地）。** 共享 base 将会话存储默认为全局 harness home（`$DSH_HOME/sessions`，默认 `~/.dsh/sessions`），而出厂 Web overlay 曾给 json 后端相对根 `./.storages`：`workspace.json` 和 `session_projcache.json` 落在 `<启动目录>/.storages/` 下——从两个不同目录启动，会话相同，工作区注册表和投影缓存却各是一份。overlay 现把 `storage-json.root` 锚定到 `$DSH_HOME/storages`，`JsonStorageBackend` 在构造时解析配置根并固定已观察目录的身份。现行行为由上方链接的源码与文档说明。
...
- **已落地**：`JsonStorageBackend` 在构造时对配置根 `resolve` 一次，后续 `process.cwd()` 变化不得把一个后端劈到多个根下；它还在发布前后核验已观察根目录的身份。
...
- 从任意目录启动 `dsh` 都读写同一份 `$DSH_HOME/storages/*.json`（默认 `~/.dsh/storages`）——已由 overlay 表达式与构造时根目录解析满足，按行覆盖走个人 config.yaml patch 层。

...
**Make release membership explicit.** The DSH `ReleaseFamily`, DSH version-bump command, and legacy npm-baseline command exclude the Saki family even though their discovery scans include `packages/saki/*`; release selection and version updates use the shared product classifier instead of a broad directory glob. Baseline packing consumes only the exact DSH and vendor directories in the discovered publication set. Existing DSH publication semantics remain owned by the [npm release sequences](../process/2026-08-10-npm-release-sequences.md); publishing Saki requires a separate release-family decision rather than inheriting DSH membership from its workspace location.
...
**Keep composition evidence slice-owned.** `@breakfastdapaidang/saki-bundle` owns an empty Cordis base config, one patch layer, and the repository-local launcher. The patch mounts only implemented product slices and a stable readiness row. Its current composition uses JSON as the default storage backend; routes the Saki control-plane, Host Execution, and Installation State Generation domains into one manifest-selected SQLite generation; and mounts Session persistence, Workspace, local filesystem and subprocess providers, the Local Host execution provider, the Windows DPAPI credential Provider and Product GitHub App on Windows, the loopback Host transport, `saki-control-plane`, and `saki-host-api`. The launcher prepares the selected Installation State Generation before boot, emits readiness only after `boot()` completes its entry-activation audit, and remains alive for the Host lifecycle; a reporting failure disposes the application and enters the launcher failure path. Later packages arrive with independently testable product slices; planned names do not justify placeholder directories.
...
The bundle contains local identity and access, Installation State Generation maintenance, existing-directory inspection and Project registration, GitHub synchronization with a confirmed Board, and [recoverable Work Item Create and Move](2026-08-16-saki-recoverable-github-work-item-mutations.md). On Windows, its Product GitHub App resolves its private key through the DPAPI credential Provider. It still has no Agent, model, or rendered Web behavior. Each later slice decides whether it owns reusable Harness capability in a DSH group or Saki product semantics under `packages/saki/`; neither the accepted architecture nor a planned package name creates a package by itself.

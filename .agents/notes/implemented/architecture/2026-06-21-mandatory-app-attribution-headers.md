...
- **pi-ai has first-class header transformation, but carriers still own the final wire.** `@earendil-works/pi-ai`'s `Models.streamSimple` applies `transformHeaders` after resolved authentication and deployment headers, so the library-backed adapter can enforce attribution across generic carriers. A carrier may still rewrite the transformed value afterward; the current Codex carrier does, so production adoption waits for an upstream correction rather than treating adapter-level transformation as final-wire proof.
...
- `dsh-llm-pi-ai` enforces the same `User-Agent` through pi-ai's `transformHeaders`, and generic provider mock-server suites assert the exact wire value. The current Codex carrier is not claimed as compliant and remains unavailable for production adoption until an upstream release preserves the value and passes exact-wire verification.
...
**Header support differs by client library and carrier.** The hand-rolled adapter sets headers directly; the pi-ai-backed adapter depends on `Models.streamSimple` continuing to apply `transformHeaders` after resolved authentication and deployment headers, and on the selected carrier preserving the result. Generic wire tests guard the adapter path. A carrier that rewrites the value remains blocked until an upstream release and exact-wire fixture prove compliance; the current Codex carrier is the known instance. This is useful pressure on the abstraction: a provider path that cannot preserve mandatory headers cannot fully implement the Harness LLM contract.

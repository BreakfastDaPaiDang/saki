...
Commands run through a wrapper that resets `$LASTEXITCODE` (assignable, verified), invokes the body via `Invoke-Expression` in a backtick-escaped double-quoted string (`quoteForPwsh`: backtick, quote, `$`, CRLF, and ESC escapes, so no raw control characters ride the input line and the wrapper survives ConstrainedLanguage), and reports the exact native exit code, `1` for a terminating PowerShell error, or `0` for success. The PTY path echoes the submitted wrapper back into the stream, so extraction strips the wrapper source from captured output; the echo can never fabricate completion because the status regex needs digits immediately after the END nonce and the echo continues with quote characters. The prompt function installs the tool's own prompt (`__DSH_PERSISTENT_PWSH_PROMPT__ `) over the backend bootstrap value, the same two-layer structure as bash.
...
- **Waiting out Unix cursor queries.** Rejected: the first .NET cursor request waits longer than the configured startup deadline, later cache misses recur after the controlled OSC prompt invalidates cursor tracking, and readiness inspection can mistake that protocol read for command completion. A protocol response fixes the missing terminal behavior instead of weakening deadlines.
- **Redirected-stdin modes or a private command dispatcher.** Rejected: `-File -` routes terminal input through the stdin-file reader, where a Unix carriage return blocks and CRLF becomes two native-child lines; `-Command -` rejects terminal stdin. A private dispatcher would need to recreate multiline parsing, Ctrl-C, errors, `exit`, scope, and native-child stdin behavior already supplied by ConsoleHost's prompt loop.
...
**PowerShell host prompts fail instead of waiting.** The default `-NonInteractive` host rejects `Read-Host` and confirmation UI. Persistent state and native foreground child input remain available through the PTY.

...
The layout declares a root-scoped keyed `main` slot. The reserved `conversation` key belongs to the Conversation plugin, whose `main.conversation` child retains optional-Session binding. Other main entries receive no implicit Session binding. The Conversation remains mounted while hidden to preserve unfinished drafts and local viewing state; the [Saki shell registration](../feature/2026-08-27-saki-web-shell-registration.md) uses these keys and restores its own persisted selection after registration.

...
Windows 提供方报告来源 `windows-dpapi-current-user` 和 Credential Protection Level `local-user-trust`。它的版本 1 文档只包含 `version` 与 `records`；每条记录只包含类型 `dpapi-ng-local-user` 和采用规范 base64 的密文。解析会拒绝未知字段、无效引用、不支持的版本、空或非规范密文，以及所有其他记录类型。该提供方不提供环境、明文、经典 DPAPI 或机器作用域回退。
...
`describe(ref)` 返回引用、配置状态、来源、Credential Protection Level、可写性、健康状态和观测时间，不返回明文或密文。复制、损坏、采用经典 DPAPI 或作用域不同的记录仍为已配置，但报告 `unavailable`。Host API 通过显式允许列表映射这些字段；提供方对象上的额外字段不会进入读取响应。具名的只写 `credentials.set` 请求会把明文传给提供方，但绝不会回显。原生层与提供方会在失败时丢弃任意底层异常，因此诊断与错误原因无法保留凭据输入。

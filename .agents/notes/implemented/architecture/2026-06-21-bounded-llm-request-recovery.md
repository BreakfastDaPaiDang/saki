...
- The headless defaults fixture covers 60 ms and 500 ms SSE-comment gaps with a 10-second idle budget, leaving scheduling margin across the server and app processes. It waits for the title request before completing the main response and asserts exactly one main request plus one title request. Adapter fake-clock tests own precise watchdog expiry and comment rearming; the assembled fixture owns default request values and delayed transport completion.

...
本决定只实现更广的[控制面能力 seam](../../proposed/architecture/2026-08-18-saki-control-plane-capability-seams.zh.md)、[稳定 Resource Binding](../../proposed/architecture/2026-08-18-saki-stable-resource-bindings.zh.md)与[可恢复 Control Intent](../../proposed/architecture/2026-08-18-saki-recoverable-control-intents.zh.md)提案中的 Host 检查与首次登记子集。独立的[polling-first GitHub 同步](2026-08-18-saki-polling-first-github-synchronization.zh.md)实现完整 GitHub read、scan 与 Board checkpoint；[可恢复 GitHub Work Item mutation](2026-08-16-saki-recoverable-github-work-item-mutations.zh.md)实现 Create 与 Move；[结构化 Git 决策](2026-08-28-saki-recoverable-structured-git-operations.zh.md)则以已绑定状态、有界 Diff、直接 mutation 与持久 Host Operation 扩展 Host Execution。三个更广泛提案保持 proposed，因为剩余 GitHub write 与 mapping repair、Model Account 与 Image Generation 能力 seam、重绑定、退役、repair、Execution Lease、后继 Session、automated dispatch、补偿和其他 Intent 家族尚未实现。
...
已认证 Host Operator 可以检查一个已有本地 Git worktree、确认其有界检查证据、只登记一次，并在重启后读取带 revision 的 Project-index 与 Development-Workspace Projection。恢复机制增加了持久中间记录与重复 Host 检查，但它会隔离歧义结果，而不是猜测外部 effect 已失败。因此，预期 revision 的失败方可能留下没有对应 Saki Project 或 Resource Binding 的可复用 DSH Workspace；登记操作不会 repair 或清理该可复用 Workspace。登记把调用方 locator 作为不可变重放内容保存，并把可信观察作为私有控制面恢复证据保存；浏览器响应、诊断与快照只暴露经过净化或规范化的值。

登记操作保持有意收窄。它不会创建或移动 repository、重绑定或退役 Resource Binding、启动 Agent Run、修改 Git，或解决进入 reconciliation-required 的登记。独立 Host Execution 方法依据[结构化 Git 决策](2026-08-28-saki-recoverable-structured-git-operations.zh.md)实现已绑定状态、Diff、stage、unstage 与 Commit；剩余能力需要各自的权限检查、持久证据与已实现 Agent Note。

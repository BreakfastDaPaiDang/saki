...
The Windows Provider reports source `windows-dpapi-current-user` and protection level `local-user-trust`. Its version-2 document contains `version`, `refs`, and `records`. A reference entry contains only kind `dpapi-ng-local-user` and canonical-base64 ciphertext; a durable-record entry also contains the safe `recordKind` tag required for descriptions and listings. Parsing rejects unknown fields, invalid references or credential keys, unsupported versions, empty or non-canonical ciphertext, unsupported protection kinds, and record tags outside the shared `CredentialRecord` union. The Provider has no ambient, plaintext, classic-DPAPI, or machine-scoped fallback, and does not accept the pre-release version-1 format.
...
Every operation re-reads and validates the Host-local document. Writes serialize within one Provider instance, re-read under the cross-process `dsh-atomic-write` lock, sort both key spaces, and commit atomically. `modifyRecord` keeps its read, owner callback, and replacement under that lock so separate Provider instances cannot lose each other's updates. Disposal drains the active write and refuses queued or captured-service writes that have not started. An absent `unset` or `deleteRecord` emits no update.

`describe(ref)` returns reference, configured state, source, protection level, writability, health, and observation time without plaintext or ciphertext. A copied, corrupt, classic, or differently scoped reference remains configured but reports `unavailable`. `describeRecord` and `listRecords` expose only credential keys, configured state, writability, and the clear record tag; `readRecord` decrypts and validates the full record and requires its encrypted tag to match the clear tag. The Host API maps safe fields through an explicit allowlist; extra fields on a Provider object do not cross read responses. Its named write-only `credentials.set` request carries plaintext to the Provider and never echoes it. Native and Provider failures discard arbitrary underlying exceptions so diagnostics and error causes cannot retain credential input.
...
On Windows, `@breakfastdapaidang/saki-bundle` composes this Provider and the Product GitHub App. GitHub synchronization configuration persists only the private-key `CredentialRef`; every Product App read and scan calls `resolveRequired(ref, 'local-user-trust')` before authentication and keeps the resolved value within that operation. Missing or differently protected values produce a bounded `auth-unavailable` failure. No Provider Account Profile implementation, Codex or Kimi authorization-completion flow, or dispatch-admission consumer composes the Provider yet, and Host replacement does not open an Intervention Request or expose credential health in the product UI. Those remaining Saki 0.1.0 obligations retain the same operation-boundary and safe-observation requirements.
...
Shared credential tests pin explicit protection metadata and fail-closed exact-level resolution for missing values and different levels. Deterministic native-adapter tests pin descriptor verification, handle and allocation cleanup, overwrite-before-free ordering, zero-length outputs, native failures, invalid UTF-8, empty plaintext, and diagnostic redaction. Real Windows tests pin CNG DPAPI `LOCAL=user` ciphertext-at-rest and restart round trips for references and durable records, concurrent record mutation, safe description and listing, deletion, disposal, and strict JSON validation; they then relabel genuine CNG `LOCAL=machine` and classic LocalMachine blobs and prove that both remain unavailable and cannot resolve. A real Loader composition runs from source and built packages, while a keyless runnable snapshot pins the Host safe-view response fields. Host API tests inject extra plaintext and ciphertext properties into Provider observations and prove that neither reaches read responses. The affected credential packages retain full statement, branch, function, and line coverage on Windows.

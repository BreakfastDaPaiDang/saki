...
The cost is duplication: tokens, primitives, and a mock shell mirror DSH instead of reusing it, and the prototype is deleted or superseded when K1 lands the real `main` path — including the prototype-only scenario tool bar, which production does not inherit. The standalone app also relies on its own `npm run build` and `validation/validate.mjs` as its only quality fence beyond the repo-wide staged gates — acceptable for a review artifact, wrong for anything that ships.

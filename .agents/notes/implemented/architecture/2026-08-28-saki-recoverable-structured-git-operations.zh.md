...
Host Operation 接受直接 `control-intent` source，以及[手动 Give-to-Agent](../feature/2026-08-18-saki-manual-give-to-agent-dispatch.zh.md)实现的 `execution-dispatch` source。两者复用 `prepareOperation` 和同一 Host Operation 生命周期。直接 Git mutation 在其操作及恢复期间持有 Binding 预留；每条 Agent Dispatch 独立拥有自身准入。

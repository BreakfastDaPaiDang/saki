...
- headless 默认值 fixture 以 10 秒空闲预算覆盖 60 ms 和 500 ms 的 SSE 注释间隔，为服务器与应用进程之间的调度留出余量。它在完成主响应前等待标题请求，并断言恰好只有一次主请求和一次标题请求。适配器假时钟测试负责精确的看门狗到期与注释重新布防；组装 fixture 负责默认请求值与延迟传输完成。

...
Every `pnpm/action-setup` step in [the primary CI workflow](../../../../.github/workflows/ci.yml) uses a runner-private destination. Linux and Wine jobs set `dest: ${{ runner.temp }}/setup-pnpm`; the native Windows job uses `${{ runner.temp }}/setup-pnpm-js` for its JavaScript installer. Each runner owns its temporary directory, so one setup cannot replace another runner's install directory. Persistent store reuse remains separate through `PNPM_CONFIG_STORE_DIR`, as established by the [pnpm provisioning decision](../process/2026-07-26-pnpm-action-setup-for-symmetric-ci-caching.md).

[The workflow regression test](../../../../scripts/ci-workflow.spec.ts) discovers every `pnpm/action-setup` step in `ci.yml` and rejects a destination that does not match its job class. This keeps newly added jobs inside the same isolation boundary.

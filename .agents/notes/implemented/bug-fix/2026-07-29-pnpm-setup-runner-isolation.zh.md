...
[主 CI 工作流](../../../../.github/workflows/ci.yml)中的每个 `pnpm/action-setup` 步骤都使用 runner 专属目标目录。Linux 与 Wine 作业设置 `dest: ${{ runner.temp }}/setup-pnpm`；原生 Windows 作业的 JavaScript 安装器使用 `${{ runner.temp }}/setup-pnpm-js`。每个 runner 独占自己的临时目录，因此一个设置过程无法替换另一个 runner 的安装目录。持久 store 的复用仍由 `PNPM_CONFIG_STORE_DIR` 独立处理，遵循 [pnpm 配置决策](../process/2026-07-26-pnpm-action-setup-for-symmetric-ci-caching.zh.md)。

[工作流回归测试](../../../../scripts/ci-workflow.spec.ts)会找出 `ci.yml` 中的每个 `pnpm/action-setup` 步骤，并拒绝与其作业类别不匹配的目标目录。这可确保后续新增的作业也处于同一隔离边界内。

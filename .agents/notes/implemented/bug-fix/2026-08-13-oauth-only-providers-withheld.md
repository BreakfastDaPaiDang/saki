...
The configurable-provider plugin does not attempt OAuth authorization. It needs a persistent credential store, a login flow, and a surface to run it from; none of those belongs to this generic settings offer, and shipping the offer without them is what produced the report. A specialized `PiAiAdapter` owner may supply the store together with its own authorization lifecycle without changing this directory decision.

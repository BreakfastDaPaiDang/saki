...
手动 Give-to-Agent 最初通过 `saki_control_plane@7`、`saki_host_execution@2` 与 `saki_storage_generation@5` 落入状态版本 7。当前状态版本 10 使用 `saki_control_plane@10`、`saki_host_execution@4` 与 `saki_storage_generation@8` 保存这些记录；冻结的版本 7 domain schema 保留原始手动启动格式。v7-to-v8 migration 增加显式 Assignment ownership、Run waiting 与 resume-pending 状态、Intervention table 和 answer message source。v8-to-v9 migration 增加 Delivery record、action 与 Push Host operation format，但不改变保留的原始启动 request 或 evidence。v9-to-v10 migration 校验历史 Agent ownership，把准入修订号保留到各自的 Dispatch，并清除 Agent 持有的 Binding 预留；手动 Git 预留和精确 Host request、Session、输入与 Run evidence 保持不变。

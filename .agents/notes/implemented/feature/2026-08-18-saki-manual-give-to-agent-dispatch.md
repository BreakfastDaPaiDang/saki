...
State version 7 carries the manual-Agent records in `saki_control_plane@7`, the `StartAgentRun` Host record in `saki_host_execution@2`, and `saki_storage_generation@5`. The adjacent migrations add empty manual-Agent tables and default Agent Profile references, and migrate retained Host v1 records to v2 without changing their immutable request or evidence.
...
This decision partially supersedes only the manual Ready-to-Run parts of the broader [dispatch, intervention, and Attention Inbox](../../proposed/architecture/2026-08-18-saki-durable-dispatch-intervention-and-attention.md), [fenced dispatch admission](../../proposed/architecture/2026-08-18-saki-fenced-idempotent-dispatch-admission.md), [recoverable Control Intent](../../proposed/architecture/2026-08-18-saki-recoverable-control-intents.md), [stable Resource Binding](../../proposed/architecture/2026-08-18-saki-stable-resource-bindings.md), and [Work Session lineage](../../proposed/architecture/2026-08-17-saki-work-sessions-over-dsh-lineage.md) notes. Those proposals remain active because they also cover automatic claiming, intervention and attention, generalized effects, rebind and retirement, or multiple Sessions and coordinators.

## Consequences

The manual path starts no model generation before an accepted explicit Intent, preserves one writer across crashes, and can take over an existing repository only after fresh Git and branch-safety evidence. Recovery pays for a multi-record state machine and may require operator reconciliation instead of maximizing availability. Automatic claiming, production provider authorization, credential and account health, intervention, and general multi-Dispatch Run orchestration remain outside this slice.

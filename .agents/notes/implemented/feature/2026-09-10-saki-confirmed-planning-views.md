...
**Disable refresh while reading cached facts.** A watch invalidation can disable the button between pointer-down and pointer-up, preventing any interactive request. The complete Board then remains unchanged until the next scheduled poll. The controller already owns request supersession, so button availability does not need a second request lifecycle.
...
The planning browser regression holds a watch-triggered cached request between pointer-down and pointer-up, observes the interactive request, and requires the newly created Issue in a complete Board. Its [diagnostic artifacts](../../../../packages/saki/bundle/README.md#planning-browser-diagnostics) retain browser requests, Host responses, Provider scans, and publication observations. This reproduces the missing-request and future-poll state reported in [Issue 107](https://github.com/BreakfastDaPaiDang/saki/issues/107) without relying on scheduler timing.

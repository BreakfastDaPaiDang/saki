...
2. Coverage callers may set `DSH_COVERAGE_TEST_TIMEOUT_MS` to budget tests, expect.poll, and hooks together, as defined by [the Windows hook budget note](../testing/2026-08-29-windows-lane-hook-and-lefthook-budget.md). The [Saki Actions cost policy](2026-08-18-saki-actions-cost-policy.md) excludes the upstream master standby and owns active runner allocation.
...
The generator resolves installed metadata from one manifest load per `render()` call, and clears the name-keyed linked-manifest cache at the start of each call so the cache cannot outlive the map it was resolved from. `THIRD_PARTY_NOTICES.md` bytes are unchanged; the freshness spec still compares `render()` against the committed document.

# Agent Note: Wine 合并门禁与手动原生 Windows 验证
...
PR 必需的 Windows 判定既需要快速的 win32 工具链信号，也不能让聚合流程等待稀缺的 Windows 容量。Wine 提供这项关键路径信号，但它运行在 Linux 内核与区分大小写的 ext4 之上，采用 hoisted 依赖布局，且无法证明 NTFS、DACL、ConPTY、崩溃持久性或原生进程行为。因此，Saki 为高风险改动与发布检查点保留显式的完整真实 Windows 诊断，而不让每个 PR revision 都为其付费。
...
原生通道的工作进程、分片、顶层门禁并发数与测试预算由 `ci.yml` 的 `windows-native.env` 定义。每个 Vitest 项目都使用 fork 进程，因为 Node 24 的 CJS lexer 致命故障可在 Windows 与 POSIX 的共享工作线程中复现。并发数调整必须以完整原生清单的测量结果为依据；显式测试预算保留原有断言结果，120 分钟作业截止时间仍会约束卡死的运行。LSP 源码与进程内 ACL 沙箱源码仍计入 Windows 覆盖率分母；只作为子进程执行的 runner 入口由 runner 套件验证。

历史专用运行器与标准运行器测量仅用于校准清单，不代表当前资源分配。标准 Windows 运行器上，双覆盖率工作进程与双门禁预算曾用 31 分 22 秒完成完整套件，其中门禁命令耗时 29 分 40 秒。该结果证明了当时的配置，不能证明调整并发数后的可靠性。
...
**让原生 Windows 成为 `all checks passed` 的依赖项。** 这会为聚合流程提供保真度最高的 Windows 判定，但也会让每次合并等待最慢的托管作业与 Windows 容量。独立的手动结果保留该诊断，同时不改变现有必需路径。

**完全移除原生 Windows 验证。** Wine 能快速触达阻断性 win32 工具链分支，但即使真实 NT、NTFS、PowerShell、进程或原生插件约定已经损坏，也可能报告绿灯。手动套件在显式检查点保留这项证据。
...
**要求使用组织自有的 16 核运行器。** 它完成清单的速度快于可移植镜像，但 Saki 不提供该运行器池。标准 `windows-latest` 运行器较慢，却无需仓库外部配置即可获得；对于手动诊断而言，这是更强的约束。
...
Wine 保留必需聚合流程现有的关键路径和作业身份。绿色的 `all checks passed` 结果不包含原生 Windows 结论；评审者与发布操作者在需要该证据时 dispatch 独立原生套件。

尽管如此，每次显式原生 dispatch 都会获得真实 NT 内核、NTFS、PowerShell、Windows 进程、原生插件和受支持源码覆盖率信号。原生作业会重复设置流程与两项阻断构建，在标准镜像上明显更慢；但它也会暴露兼容性通道掩盖的路径、watcher、生命周期与 fixture 缺陷。

维护者必须保留两种有意设计的执行拓扑：Wine 快照使用 Linux 安装加 hoisted 布局来触达 win32 二进制文件，而手动原生作业在标准 Windows 上使用不可变工作区。任一作业独有的失败都必须依据该边界分类，不得削弱或静默跳过。

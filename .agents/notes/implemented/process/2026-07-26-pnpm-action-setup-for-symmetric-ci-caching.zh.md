...
- **对称缓存**（既恢复也保存）：带 `cache: pnpm` 的 `actions/setup-node`——由手动 dispatch 触发的 `e2e.yml`、由版本 tag 或手动 dispatch 触发的 `docs-pages.yml`、`pi-ai-provider-e2e.yml`、`build-exe-for-python-sdk.yml`，以及 `ci.yml` 的 node-compat 与两个 benchmark 作业。larger-runner benchmark 通过条件化的 `cache:` 输入让 store 缓存仅限 Linux；consolidated benchmark 在两个平台上都启用缓存。
- **只恢复不上传**（手写的 `actions/cache/restore` 步骤）：三个已就绪 PR 的必需 Linux 作业只恢复不保存，把缓存压缩与上传挡在它们的延迟敏感路径之外——这种不对称是 `setup-node` 的缓存无法表达的。每个作业都在 action 可替换的安装目录之外配置 store，并在存在兼容的前缀缓存时使用它。Saki 没有自动运行的默认分支生产者，因此缓存不存在或被清理后的冷安装路径必须继续受支持。
- **仅限本次运行或无缓存**：由 Wine 承载的必需 Windows 作业可以只为同一 PR ref 的重新运行保存其依赖归档；原生 Windows 手动套件与由发布 tag 触发的 `sandbox.yml` 使用冷 pnpm store 或 runner 本地 store。解压含有大量文件的 pnpm store 可能比在 Windows 上进行一次全新安装更耗时，因此原生 Windows 不传输该归档。
...
- **增加自动运行的默认分支缓存生产者。** Saki 不予采用：由 master 推送触发的生产者会在每次合并后消耗分钟数，却不会参与合并裁决。只恢复不上传的作业必须在没有兼容缓存时仍然正确；未来若要增加生产者，需要单独的成本证据。
...
- `pnpm/action-setup` 每次运行都会删除其安装目录，并把默认 store 放在由此产生的 `PNPM_HOME` 下。因此，使用只恢复不上传缓存的 Linux 作业会把 `PNPM_CONFIG_STORE_DIR` 设为 `$HOME/.local/share/pnpm/store`，置于 action 目录之外，并解析任何兼容既有缓存所使用的稳定路径。

...
下文基准数据继续作为操作者显式 dispatch 比较时选择大型托管运行器的证据。Saki 不提供被测的企业级或自托管池：已就绪 PR 使用 GitHub 标准托管 Linux 加 Wine，完整的原生 Windows 清单依据 [Saki Actions 策略](2026-08-18-saki-actions-cost-policy.md)在 `windows-latest` 上手动运行。标称核心数仍不足以成为改变分配方式的证据；候选环境必须完成同一份仓库工作负载，并报告实测的处理器与内存容量。

必需主路径保留在[可移植的标准运行器边界](2026-07-23-portable-required-pull-request-ci.md)上，[串行参考流程](2026-07-21-serial-cross-platform-ci-reference.md)则保留完整的诊断定义，但不再由 `master` 自动触发。`suite=larger-runner-benchmark` 比较任意已预配规格上相互独立的关键通道，`suite=consolidated-runner-benchmark` 则比较完整聚合流程。两者都是显式手动套件；默认手动选项是可移植的 `windows-native` 诊断，因此常规 dispatch 不会排队等待缺失的大型运行器标签。

原有的门禁级和粗粒度主流程分片作业已从工作流中移除。相应的静态、lint、覆盖率、快照和场景分片选择器也已从仓库中移除，因此未使用的诊断路径无法继续维系第二套 CI 架构。

Saki 必需的 Linux 主流程使用 3 个相互独立的标准托管作业。覆盖率单独运行，并设有自己的工作线程上限；静态调度器负责不消费生成输出的源码和文档门禁。第三个作业负责唯一一次 Linux 构建，随后让 lint、Node 24 运行时兼容性、依赖构建产物的快照、文档类型检查和所有产物消费方基于该目录树启动。这种[消费方独立构建](2026-07-30-independent-ci-consumer-build.md)使 3 个作业都能立即请求运行器，而无需重复编译或传输仅供本次运行使用的产物。生成的 NodeNext 消费方目录不会纳入 Oxlint 的文件发现范围，因为这些进程重叠执行时，产物检查会删除这些目录。pnpm store 会得到恢复，但缓存上传不会进入拉取请求关键路径；Oxlint 没有由仓库管理的结果缓存。性能报告采用每个作业从 `startedAt` 到 `completedAt` 的区间；运行器排队延迟是容量证据，而非仓库执行时间。下文 32 核数据描述历史上被测的企业级拓扑与显式比较套件，而非 Saki 的必需资源分配。
...
在被测的企业级拓扑中，Windows 通过一次 32 核环境设置同时承载阻塞性构建、生产网站与观测性构建产物约定，重复的 lint、覆盖率和快照清单则由 Linux 负责。Saki 保留这些测量结果，但将[双 Windows 验证](2026-08-08-native-windows-pull-request-ci.md)实现为必需的 Wine 作业加显式的标准托管原生套件。

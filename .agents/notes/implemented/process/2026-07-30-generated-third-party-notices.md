...
A dependency or Saki Development Skill Pack provenance edit now carries a regenerated notices file into the same commit. Contributors pay one generator run — about a second — on commits that touch an input manifest, and nothing on any other commit. Committing with hooks disabled defers the cost to a test-lane failure that names the command.

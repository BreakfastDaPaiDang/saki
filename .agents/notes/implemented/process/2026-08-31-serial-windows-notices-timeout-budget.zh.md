...
2. 覆盖率调用方可以通过 `DSH_COVERAGE_TEST_TIMEOUT_MS` 同时设置测试、expect.poll 与 hook 预算，机制由 [Windows hook 预算 note](../testing/2026-08-29-windows-lane-hook-and-lefthook-budget.zh.md) 定义。[Saki Actions 成本策略](2026-08-18-saki-actions-cost-policy.zh.md) 排除上游 master 备用 lane，并拥有当前 runner 分配。
...
generator 每次 `render()` 调用只加载一次 manifest 来解析已安装元数据，并在调用开头清空按名字作键的 linked-manifest 缓存，使缓存不会活过它解析自的那份 map。`THIRD_PARTY_NOTICES.md` 字节不变；新鲜度 spec 仍把 `render()` 与已提交文档逐字节比较。

...
Local Host 生命周期、control-plane Project、Host transport 和 Work 浏览器夹具在 Windows 上为 inventory 与 baseline capture 提供 120 秒预算。完整 Windows 源码模式序列为生命周期用例提供十分钟，为反复注册、检查 Git 和重启控制面的 Branch Delivery 路由用例提供三十分钟；原生已绑定仓库消失用例在故意删除之前约需一分钟。这些预算保留真实托管 Git 路径，不改变生产默认值。

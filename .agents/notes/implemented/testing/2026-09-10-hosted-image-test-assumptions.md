...
The upstream Linux and Windows coverage lanes grant `DSH_COVERAGE_TEST_TIMEOUT_MS: '90000'` because the disposal cases in `subprocess-local` and `bash-sandbox` exceed the 5000ms default when the lane's partitions, workers, and sibling gates share one host.

Saki assigns that budget to `node-24-coverage.env`; the static job neither runs coverage nor passes its environment to sibling jobs. Without the variable in the coverage job, the generated Host schema dependency test reached Vitest's 5000ms default in [run 34738985999](https://github.com/BreakfastDaPaiDang/saki/actions/runs/34738985999/job/103675323633). The [workflow regression](../../../../scripts/ci-workflow.spec.ts) checks the owning job so the coverage coordinator receives the paired test, polling, and hook budgets.
